...
- Before creating a commit, pull request, or deployment, run the complete `pnpm run test:full` from the repository root with all required environment variables. Do not commit or open/update a PR while any phase fails: diagnose the failure, fix the underlying issue, and rerun the complete suite. Targeted tests are supplementary and never replace the full gate; report any environment-blocked phase explicitly instead of claiming the gate passed.
