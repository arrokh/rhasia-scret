...
- Treat `apps/web/src/app/ui-preview/**` as first-class development preview routes. When changing shared app-shell, footer, navigation, form, state, or localization behavior, inspect every affected preview route and update its route composition, preview harness, fixtures, and matching tests so the preview reflects the correct production implementation. Keep preview-only data synthetic and ciphertext-free.
...
- Before creating a commit, opening or updating a pull request, deploying, or merging any branch into `main`, run a fresh complete `pnpm run test:full` from the repository root with all required environment variables after the final code or configuration change. An agent must never merge to `main` unless this exact local gate passed; diagnose failures, fix the cause, and rerun the complete suite. Targeted tests are supplementary and never replace the full gate; report any environment-blocked phase explicitly instead of claiming the gate passed. This verification requirement never authorizes a migration operation; obtain separate explicit human confirmation before changing any database.
- Hosted and required local browser verification currently focuses on Chromium only. Keep Firefox and WebKit entries commented out in `.github/workflows/ci.yml` and Playwright project configuration, and do not add or require Firefox/WebKit tests unless this support decision changes.
