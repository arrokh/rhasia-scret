...
export async function rotateUserEncryptionIdentityWithCrypto(
  ...
  const wrapped = await Promise.all(
    wrappedVaultKeys.map(async (packageBytes) => {
      ...
      try {
        return crypto.serializeKeyWrapEnvelope(
          await crypto.wrapKeyForRecipientWithContext(vaultKey, identity.publicKey, context),
        );
      } finally {
        vaultKey.fill(0);
      }
