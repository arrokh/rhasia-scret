...
} from "../../../shared/application/archive-parameters";
...
export function createVaultArchiveProtocol(crypto: ClientCryptoPort) {
  return {
    createEncryptedVaultArchive: async (
      ...
      if (!normalizedName || normalizedName.length > MAX_VAULT_NAME_LENGTH)
        throw new Error("Encrypted vault export is invalid.");
      if (accountPlaintexts.length > MAX_VAULT_ARCHIVE_ACCOUNTS)
        throw new Error("Encrypted vault export is too large.");
      const accounts = accountPlaintexts.map((plaintext) => {
        if (plaintext.length === 0 || plaintext.length > MAX_ARCHIVED_ACCOUNT_BYTES)
          throw new Error("Encrypted vault export is too large.");
        return bytesToBase64(plaintext);
      });
      const plaintext = new TextEncoder().encode(
        JSON.stringify({ version: FORMAT_VERSION, vaultName: normalizedName, accounts } satisfies VaultExportPayload),
      );
      try {
        if (plaintext.length > MAX_ENCRYPTED_VAULT_ARCHIVE_BYTES - 29)
          throw new Error("Encrypted vault export is too large.");
        return crypto.serializeEncryptedEnvelope(
          await crypto.encryptPayloadWithContext(archiveKey, plaintext, {
    ...
    openEncryptedVaultExport: async (
      archiveKey: Uint8Array,
      archive: Uint8Array,
...
function parsePayload(value: unknown): VaultExportPayload {
  ...
  if (
    Object.keys(payload).sort().join(",") !== "accounts,vaultName,version" ||
    ...
    !payload.accounts.every(validArchivedAccount)
  )
    invalid();
