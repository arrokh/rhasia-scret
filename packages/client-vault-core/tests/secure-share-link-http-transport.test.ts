...
describe("SecureShareLinkHttpTransport", () => {
  it("creates a link with normalized recipient and ciphertext-only material", async () => {
    ...
    await expect(protocol.create("vault_1", { recipientEmail: " Recipient@Example.test ", linkVerifier: verifier, encryptedPackage: ciphertext }))
    ...
    expect(transport.requests[0]).toEqual({
      ...
      body: JSON.stringify({ recipientEmail: "recipient@example.test", linkVerifier: verifier, encryptedPackage: ciphertext }),
      cache: "no-store"
    });
  });
  ...
  it("looks up and redeems a one-time link without sending its secret", async () => {
    ...
    const protocol = new SecureShareLinkHttpTransport(transport);
    await expect(protocol.lookup(verifier)).resolves.toEqual({ id: "invitation_1", vaultId: "vault_1", encryptedPackage: ciphertext });
    ...
    expect(String(transport.requests[1]?.body)).toBe(JSON.stringify({ invitationId: "invitation_1", encryptedVaultKey: ciphertext, keyVersion: 1 }));
  ...
  it("cancels the exact pending invitation", async () => {
    ...
    expect(transport.requests[0]).toEqual({ url: "/api/shared-vaults/vault_1/share-links/invitation_1", method: "DELETE", cache: "no-store" });
  });

  it("strictly parses responses and preserves expiry/error statuses", async () => {
    const malformed = new SecureShareLinkHttpTransport(new StubTransport([response(200, { id: "invitation_1", vaultId: "vault_1", encryptedPackage: ciphertext, secret: "forbidden" })]));
