...
describe("HostedAuthenticatorAccountTransport", () => {
  it.each([
    ...
    await expect(protocol.create({ vaultId: "vault_1", vaultType }, { encryptedPayload: ciphertext, encryptionVersion: 1 })).resolves.toEqual({ id: "account_1", revision: 1 });
  ...
  it("preserves revisions and the Local Vault copy source marker", async () => {
    ...
    expect(JSON.parse(String(transport.requests[0]?.body))).toEqual({ accountId: "account_1", expectedRevision: 2, encryptedPayload: ciphertext, encryptionVersion: 1 });

    transport.result = response(201, { id: "account_2", revision: 1 });
    await protocol.create({ vaultId: "vault_1", vaultType: "PERSONAL" }, { encryptedPayload: ciphertext, encryptionVersion: 1, source: "LOCAL_VAULT_COPY" });
  ...
  it("requires exact empty-success statuses for delete and restore", async () => {
    ...
    const protocol = new HostedAuthenticatorAccountTransport(transport);
    await protocol.delete({ vaultId: "shared_1", vaultType: "SHARED" }, { accountId: "account_1", expectedRevision: 4 });
  ...
  it("rejects extra response fields and normalizes server failures", async () => {
    const protocol = new HostedAuthenticatorAccountTransport(new StubTransport(response(201, { id: "account_1", revision: 1, plaintext: "forbidden" })));
    await expect(protocol.create({ vaultId: "vault_1", vaultType: "PERSONAL" }, { encryptedPayload: ciphertext, encryptionVersion: 1 })).rejects.toThrow("protocol value is invalid");

    const failed = new HostedAuthenticatorAccountTransport(new StubTransport(response(409, { error: "stale_revision" })));
