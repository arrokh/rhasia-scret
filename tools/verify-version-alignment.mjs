...
const packageVersions = await Promise.all(packageFiles.map(async (relativePath) => {
...
const mismatches = versions.filter(([, version]) => version !== sourceVersion || typeof version !== "string" || !versionPattern.test(version));
...
console.info(`Release version ${sourceVersion} is aligned across the root, web, mobile, core, and Expo app configuration.`);
