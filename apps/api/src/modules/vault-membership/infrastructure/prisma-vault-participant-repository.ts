import { buildCursorPage, type CursorPageRequest } from "@api/shared/application/cursor-page";
...
export class PrismaVaultParticipantRepository implements VaultParticipantRepository {
  ...
  public async listForOwner(
    ...
    if (cursorKey?.kind !== "INVITATION") {
      ...
      const members = await this.database.vaultMember.findMany({
        where: {
          vaultId,
          role: "VIEWER",
          status: "ACTIVE",
    ...
    if (participants.length <= request.limit) {
      ...
      const invitations = await this.database.vaultInvitation.findMany({
        where: {
          vaultId,
          status: "PENDING",
          vault: { ownerId, type: "SHARED", lifecycle: "ACTIVE", deletedAt: null },
  ...
  public async cancelInvitation(ownerId: string, vaultId: string, invitationId: string): Promise<boolean> {
    const deleted = await this.database.vaultInvitation.deleteMany({
      where: {
        id: invitationId,
        vaultId,
        status: "PENDING",
