...
import { constantTimeEqual, hmacSha256, randomUuidWithoutDashes } from "@api/shared/infrastructure/crypto";
import {
  PASSWORDLESS_ISSUER,
  normalizeEmail,
...
export class PrismaPasswordlessAuthRepository implements PasswordlessAuthRepository {
  ...
  public async findOrCreateAccount(email: string, now: Date): Promise<PasswordlessAccount> {
    try {
      return await this.database.$transaction(async (transaction) => {
        ...
        const subject = randomUuidWithoutDashes();
      ...
      const raced = await this.database.passwordlessIdentity.findUnique({
        where: { normalizedEmail: email },
        include: { externalIdentity: { include: { applicationUser: true } } },
      });
      if (!raced) throw error;
      await this.database.$transaction([
        this.database.externalIdentity.update({
  ...
  public async createSession(account: PasswordlessAccount, now: Date): Promise<PasswordlessSession> {
    return this.database.$transaction((transaction) => this.createSessionInTransaction(transaction, account, now));
  }
  ...
  public async verifyAccessToken(tokenDigest: Uint8Array, now: Date): Promise<VerifiedPrincipal | null> {
    ...
    if (
      ...
      return null;
    await this.database.authSession.update({ where: { id: session.id }, data: { lastUsedAt: now } });
    return {
  ...
  private digestCredential(token: string): Uint8Array {
    return hmacSha256(this.configuration.sessionSecret, token);
  }
}
...
function randomSessionId(): string {
  return randomUuidWithoutDashes().replace(/-/g, "");
}
