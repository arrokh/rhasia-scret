...
} from "@api/shared/application/cursor-page";
...
export class PrismaVaultAuditRepository implements VaultAuditRepository {
  ...
  public async recordAccountAccess(actorUserId: string, vaultId: string, accountId: string): Promise<boolean> {
    ...
    await appendVaultAuditEvent(this.database, {
      vaultId,
  ...
  public async recordArchiveExport(ownerId: string, vaultId: string): Promise<boolean> {
    return this.database.$transaction(async (transaction) => {
      ...
      await appendVaultAuditEvent(transaction, { vaultId, ownerId, actorUserId: ownerId, action: "ARCHIVE_EXPORTED" });
      return true;
    });
  }
  ...
  public async listForOwner(
    ...
    const retainedAfterVaultPurge = vault
      ? false
      : (await this.database.vaultAuditEvent.count({
    ...
    const events = await this.database.vaultAuditEvent.findMany({
      where: {
        vaultId,
