...
import { PrismaApplicationRateLimitRepository } from "@api/modules/rate-limiting/infrastructure/prisma-application-rate-limit-repository";
...
describe("Prisma application rate-limit repository", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      const repositories = [
        new PrismaApplicationRateLimitRepository(prisma),
        new PrismaApplicationRateLimitRepository(prisma),
      ];
  ...
  it.skipIf(!process.env.DATABASE_URL)("retains only a bounded period of expired aggregate windows", async () => {
    ...
    await new PrismaApplicationRateLimitRepository(prisma).consume(user.id, "audit_event", {
      limit: 1,
      windowSeconds: 60,
    });
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      const repository = new PrismaApplicationRateLimitRepository(prisma);
