...
import { PrismaEncryptedVaultImportRepository } from "@api/modules/vault-archive/infrastructure/prisma-encrypted-vault-import-repository";
...
describe("PrismaEncryptedVaultImportRepository", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    "rejects duplicate account identifiers before opening a database transaction",
    async () => {
      ...
      await expect(new PrismaEncryptedVaultImportRepository(prisma).import(randomUUID(), request)).resolves.toEqual({
        status: "CONFLICT",
      });
    },
  );
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "atomically imports all ciphertext accounts into an owned existing destination and replays safely",
    async () => {
      ...
      const request = {
        ...
        accounts: accountIds.map((id) => ({
          id,
          encryptedPayload: bytes(`ciphertext-${id}`),
          encryptionVersion: 1 as const,
        })),
      };
      const repository = new PrismaEncryptedVaultImportRepository(prisma);

      await expect(repository.import(owner.id, request)).resolves.toEqual({
        status: "IMPORTED",
        vaultId: vault.id,
        accountIds,
        vaultCreated: false,
      ...
      expect(
        await prisma.vaultAuditEvent.findMany({
          ...
          select: { eventType: true, targetId: true },
        }),
      ).toEqual([{ eventType: "ARCHIVE_IMPORTED", targetId: null }]);
    },
  );

  it.skipIf(!process.env.DATABASE_URL)(
    "imports into an existing owned Shared Vault with one redacted audit event",
    async () => {
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "lets an add-authorized member import into an existing Shared Vault and re-authorizes replay",
    async () => {
      ...
      await expect(repository.import(member.id, request)).resolves.toEqual({
        status: "IMPORTED",
        vaultId: vault.id,
        accountIds: [accountId],
        vaultCreated: false,
      ...
      await expect(
        prisma.vaultAuditEvent.findFirst({
          where: { vaultId: vault.id },
          select: { ownerId: true, actorUserId: true, eventType: true },
        }),
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "creates a Shared Vault, owner key, all accounts, and redacted audit event in one transaction",
    async () => {
      ...
      expect(
        await prisma.vault.findUnique({
          ...
          select: {
            ownerId: true,
            type: true,
            members: { select: { userId: true, role: true, encryptedVaultKey: true } },
      ...
      expect(
        await prisma.vaultAuditEvent.findMany({
          where: { vaultId },
          select: { ownerId: true, eventType: true, targetId: true },
        }),
      ).toEqual([{ ownerId: owner.id, eventType: "ARCHIVE_IMPORTED", targetId: null }]);
    },
  );

  it.skipIf(!process.env.DATABASE_URL)(
    "rejects unauthorized destinations and conflicting identifiers without partial writes",
    async () => {
      const owner = await createUser("authorized-owner");
      const outsider = await createUser("outsider");
      const ownerVault = await createVault(owner.id, "PERSONAL");
      const conflicting = await prisma.authenticatorAccount.create({
        data: { id: randomUUID(), vaultId: ownerVault.id, encryptedPayload: bytes("existing"), encryptionVersion: 1 },
      });
      const repository = new PrismaEncryptedVaultImportRepository(prisma);

      await expect(
        repository.import(outsider.id, {
          destination: { kind: "EXISTING", vaultId: ownerVault.id, vaultType: "PERSONAL" },
          accounts: [],
        }),
      ).resolves.toEqual({ status: "DESTINATION_UNAVAILABLE" });
...
async function createVault(ownerId: string, type: "PERSONAL" | "SHARED") {
  const vault = await prisma.vault.create({
    data: { ownerId, type, lifecycle: "ACTIVE", encryptedName: bytes("encrypted-vault"), encryptionVersion: 1 },
  });
