...
describe("PrismaSecureShareLinkRepository email invitation", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    "creates an email-bound invitation before first sign-in and redeems it once after provisioning",
    async () => {
      ...
      const repository = new PrismaSecureShareLinkRepository(prisma);
      ...
      await expect(
        prisma.vaultInvitation.findUnique({
          ...
          select: { recipientEmail: true, recipientUserId: true },
        }),
      ).resolves.toEqual({ recipientEmail, recipientUserId: null });
      ...
      await expect(
        prisma.vaultInvitation.findUnique({
          where: { id: invitation.id },
          select: { recipientUserId: true, status: true, redeemedAt: true },
        }),
      ).resolves.toEqual({ recipientUserId: recipient.id, status: "REDEEMED", redeemedAt: expect.any(Date) });
      await expect(
        repository.findForRecipient({ userId: recipient.id, email: recipientEmail }, verifier),
      ).resolves.toBeNull();
    },
  );

  it.skipIf(!process.env.DATABASE_URL)(
    "atomically replaces an expired Invitation with fresh link material",
    async () => {
      const owner = await user("owner");
      ...
      const expired = await prisma.vaultInvitation.create({
        data: {
          vaultId: vault.id,
          recipientEmail: recipient.email,
          recipientUserId: recipient.id,
          linkVerifier: oldVerifier,
          encryptedPackage: bytes("old-package"),
          expiresAt: now,
        },
      });
      ...
      await expect(
        repository.findForRecipient({ userId: recipient.id, email: recipient.email }, oldVerifier),
      ).resolves.toBeNull();
      await expect(
        repository.findForRecipient({ userId: recipient.id, email: recipient.email }, replacementVerifier),
      ).resolves.toEqual(expect.objectContaining({ id: replacement.id }));
    },
  );

  it.skipIf(!process.env.DATABASE_URL)(
    "binds one pending invitation to a provisioned application user email",
    async () => {
      const owner = await user("owner");
      const recipient = await user("recipient");
      const vault = await prisma.vault.create({
      ...
      await expect(
        prisma.vaultInvitation.findUnique({ where: { id: invitation.id }, select: { recipientUserId: true } }),
      ).resolves.toEqual({ recipientUserId: recipient.id });
      await expect(
      ...
      await prisma.vault.update({
        where: { id: vault.id },
        data: { lifecycle: "DELETED", deletedAt: new Date(), purgeAfter: new Date(Date.now() + 86_400_000) },
      });
      await expect(
        repository.findForRecipient({ userId: recipient.id, email: recipient.email }, verifier),
      ).resolves.toBeNull();
    },
  );

  it.skipIf(!process.env.DATABASE_URL)(
    "reactivates a revoked membership with every personal permission override cleared",
    async () => {
      const owner = await user("owner");
      const recipient = await user("recipient");
      const vault = await prisma.vault.create({
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "does not let a recycled email override an invitation already bound to another user id",
    async () => {
      const owner = await user("owner");
      ...
      await prisma.applicationUser.update({
        where: { id: intendedRecipient.id },
        data: { email: `changed-${randomUUID()}@example.test` },
      });
      const otherUser = await user("other", originalEmail);

      await expect(
        repository.findForRecipient({ userId: otherUser.id, email: originalEmail }, verifier),
      ).resolves.toBeNull();
    },
  );
});
