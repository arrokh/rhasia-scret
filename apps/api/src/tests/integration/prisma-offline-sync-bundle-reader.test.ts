...
import { PrismaOfflineSyncBundleReader } from "@api/modules/sync/infrastructure/prisma-offline-sync-bundle-reader";
...
afterEach(async () => {
  if (userIds.length) {
    ...
    await prisma.vaultMember.deleteMany({
      where: { OR: [{ vaultId: { in: vaultIds } }, { userId: { in: userIds } }] },
    });
...
describe("PrismaOfflineSyncBundleReader", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    "reads one consistent complete authorized bundle and omits revoked or deleted Vaults",
    async () => {
      ...
      await prisma.authenticatorAccount.create({
        data: { vaultId: personal.id, encryptedPayload: opaque, encryptionVersion: 1 },
      });
      ...
      await prisma.vaultMember.update({
        where: { vaultId_userId: { vaultId: viewed.id, userId: user.id } },
        data: { canEditAccountsOverride: true, canDeleteAccountsOverride: false },
      });
      ...
      expect(bundle?.personalVault).toEqual(
        expect.objectContaining({ vaultId: personal.id, accounts: [expect.objectContaining({ revision: 1 })] }),
      );
      expect(bundle?.sharedVaults).toHaveLength(2);
      expect(bundle?.sharedVaults.map(({ vaultId, role }) => ({ vaultId, role }))).toEqual(
      ...
      changedPayload[1] = 9;
      await prisma.authenticatorAccount.create({
        data: { vaultId: personal.id, encryptedPayload: changedPayload, encryptionVersion: 1 },
      });
      const changed = await new PrismaOfflineSyncBundleReader(prisma).readAuthorizedBundle(user.id);
    ...
    data: {
      ...
      members: {
        create: {
          ...
          role,
          status: memberStatus,
          encryptedVaultKey: type === "SHARED" ? opaque : null,
          keyVersion: type === "SHARED" ? 1 : null,
        },
      },
    },
  });
