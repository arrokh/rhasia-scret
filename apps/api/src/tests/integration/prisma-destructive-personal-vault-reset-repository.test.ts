...
import { PrismaDestructivePersonalVaultResetRepository } from "@api/modules/vault-management/infrastructure/prisma-destructive-personal-vault-reset-repository";
...
async function cleanup() {
  ...
  await prisma.vaultInvitation.deleteMany({
    where: { OR: [{ vaultId: { in: vaultIds } }, { recipientUserId: { in: ids } }] },
  });
...
describe("PrismaDestructivePersonalVaultResetRepository", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    "destroys only the user's unusable material and leaves invited Shared Vault data intact",
    async () => {
      ...
      await new PrismaDestructivePersonalVaultResetRepository(prisma).reset(resettingUser.id);
      ...
      await expect(
        prisma.vault.findUnique({ where: { id: personalVault.id }, select: { lifecycle: true, encryptedName: true } }),
      ).resolves.toEqual({ lifecycle: "UNINITIALIZED", encryptedName: null });
      await expect(
        prisma.vaultMember.findUnique({
          where: { vaultId_userId: { vaultId: sharedVault.id, userId: resettingUser.id } },
          select: { status: true, encryptedVaultKey: true, keyVersion: true },
        }),
      ).resolves.toEqual({ status: "LEFT", encryptedVaultKey: null, keyVersion: null });
      await expect(
        prisma.vault.findUnique({ where: { id: sharedVault.id }, select: { lifecycle: true } }),
      ...
      await expect(
        prisma.vaultMember.findUnique({
          where: { vaultId_userId: { vaultId: sharedVault.id, userId: sharedOwner.id } },
          select: { status: true },
        }),
  ...
  it.skipIf(!process.env.DATABASE_URL)("rejects reset while the user owns an active Shared Vault", async () => {
    ...
    await prisma.vault.create({
      data: {
        ...
        type: "SHARED",
        lifecycle: "ACTIVE",
        encryptedName: bytes("owned-shared"),
        encryptionVersion: 1,
        members: { create: { userId: user.id, role: "OWNER" } },
      },
    });

    await expect(new PrismaDestructivePersonalVaultResetRepository(prisma).reset(user.id)).rejects.toBeInstanceOf(
      ActiveOwnedSharedVaultsPreventResetError,
    );
