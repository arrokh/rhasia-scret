...
import { PrismaExpiredAccountPurgeRepository } from "@api/modules/authenticator-account/infrastructure/prisma-expired-account-purge-repository";
...
import { auditPurgeAfter } from "@api/modules/audit/server";
import { PrismaExpiredVaultAuditRepository } from "@api/modules/audit/infrastructure/prisma-expired-vault-audit-repository";
import { vaultPurgeAfter } from "@api/modules/vault-management/domain/vault-retention-policy";
import { PrismaExpiredVaultRetentionRepository } from "@api/modules/vault-management/infrastructure/prisma-expired-vault-retention-repository";
import { PrismaSharedVaultRecoveryRepository } from "@api/modules/vault-management/infrastructure/prisma-shared-vault-recovery-repository";
import { PrismaVaultAuditRepository } from "@api/modules/audit/infrastructure/prisma-vault-audit-repository";
...
describe("Prisma retention purge", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    "purges only eligible accounts at their deadlines and is idempotent",
    async () => {
      ...
      await prisma.authenticatorAccount.update({
        where: { id: legacy.id },
        data: { deletedAt: new Date(now.getTime() - 31 * 24 * 60 * 60 * 1000), purgeAfter: null },
      });
      const repository = new PrismaExpiredAccountPurgeRepository(prisma);
      ...
      expect(
        await prisma.authenticatorAccount.findMany({
          where: { id: { in: [expired.id, future.id, active.id, legacy.id] } },
          select: { id: true },
        }),
      ).toEqual(expect.arrayContaining([{ id: future.id }, { id: active.id }]));
  ...
  it.skipIf(!process.env.DATABASE_URL)("sets and clears account deadlines through delete and restore", async () => {
    ...
    const repository = new PrismaSharedAccountRepository(prisma, () => deletedAt);

    await expect(repository.delete(owner.id, vault.id, account.id, 1)).resolves.toEqual({
      status: "SUCCESS",
      value: undefined,
    });
    expect(
      await prisma.authenticatorAccount.findUnique({
        where: { id: account.id },
        select: { deletedAt: true, purgeAfter: true },
      }),
    ).toEqual({ deletedAt, purgeAfter: new Date("2026-08-25T12:00:00.000Z") });
    await expect(
      new PrismaSharedAccountRepository(prisma, () => new Date("2026-08-25T11:59:59.999Z")).restore(
        owner.id,
        vault.id,
        account.id,
      ),
    ...
    expect(
      await prisma.authenticatorAccount.findUnique({
        where: { id: account.id },
        select: { deletedAt: true, purgeAfter: true, revision: true },
      }),
    ).toEqual({ deletedAt: null, purgeAfter: null, revision: 3 });
  });

  it.skipIf(!process.env.DATABASE_URL)(
    "purges migrated legacy Shared Vault deletions whose explicit deadline was not yet stored",
    async () => {
      ...
      await prisma.vault.update({
        where: { id: vault.id },
        data: { lifecycle: "DELETED", deletedAt, purgeAfter: null },
      });
      await prisma.vaultAuditEvent.create({
        data: { vaultId: vault.id, actorUserId: owner.id, eventType: "VAULT_DELETED" },
      });

      const legacyPurge = await new PrismaExpiredVaultRetentionRepository(prisma).purgeExpiredVaults(now, 100);
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "purges expired Shared Vault content while retaining owner-only audit history for one year",
    async () => {
      ...
      await new PrismaVaultAuditRepository(prisma).recordAccountAccess(viewer.id, vault.id, account.id);
      const deletedAt = new Date("2026-07-26T12:00:00.000Z");
      await expect(
        new PrismaSharedVaultRecoveryRepository(prisma, () => deletedAt).delete(owner.id, vault.id),
      ).resolves.toBe(true);

      const stored = await prisma.vault.findUniqueOrThrow({
        where: { id: vault.id },
        select: { deletedAt: true, purgeAfter: true },
      });
      expect(stored).toEqual({ deletedAt, purgeAfter: new Date("2026-08-25T12:00:00.000Z") });
      await expect(
        new PrismaVaultAuditRepository(prisma, () => new Date("2026-08-01T00:00:00.000Z")).listForOwner(
          owner.id,
          vault.id,
        ),
      ).resolves.toEqual(
      ...
      await expect(new PrismaVaultAuditRepository(prisma).listForOwner(viewer.id, vault.id)).resolves.toBeNull();

      const retention = new PrismaExpiredVaultRetentionRepository(prisma);
      ...
      const retainedAuditPage = await new PrismaVaultAuditRepository(
        prisma,
        () => new Date("2027-07-26T11:59:59.999Z"),
      ).listForOwner(owner.id, vault.id);
      expect(retainedAuditPage?.items).toHaveLength(2);
      await expect(new PrismaVaultAuditRepository(prisma).listForOwner(viewer.id, vault.id)).resolves.toBeNull();

      const ownAuditIds = (
        await prisma.vaultAuditEvent.findMany({ where: { vaultId: vault.id }, select: { id: true } })
      ).map(({ id }) => id);
      const auditRetention = new PrismaExpiredVaultAuditRepository(prisma);
      const auditBeforeDeadline = await auditRetention.purgeExpiredAuditEvents(
        new Date("2027-07-26T11:59:59.999Z"),
        100,
      );
      ...
      const repeatedAuditPurge = await auditRetention.purgeExpiredAuditEvents(
        new Date("2027-07-26T12:00:00.000Z"),
        100,
      );
      expect(ownAuditIds.every((id) => !repeatedAuditPurge.purgedIds.includes(id))).toBe(true);
      await expect(
        new PrismaVaultAuditRepository(prisma, () => new Date("2027-07-26T12:00:00.000Z")).listForOwner(
          owner.id,
          vault.id,
        ),
      ).resolves.toBeNull();
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    "never purges a Shared Vault or account that wins a concurrent restore",
    async () => {
      ...
      await prisma.authenticatorAccount.update({
        where: { id: account.id },
        data: { deletedAt, purgeAfter: deadline },
      });
      const [restoredAccount, purgedAccounts] = await Promise.all([
        new PrismaSharedAccountRepository(prisma, () => new Date(deadline.getTime() - 1)).restore(
          owner.id,
          vault.id,
          account.id,
        ),
        new PrismaExpiredAccountPurgeRepository(prisma).purgeExpired(new Date(deadline.getTime() + 1), 100),
      ]);
      ...
      await prisma.vault.update({
        where: { id: vault.id },
        data: { lifecycle: "DELETED", deletedAt, purgeAfter: deadline },
      });
      const retention = new PrismaExpiredVaultRetentionRepository(prisma);
      const [restoredVault, purgedVaults] = await Promise.all([
        new PrismaSharedVaultRecoveryRepository(prisma, () => new Date(deadline.getTime() - 1)).restore(
          owner.id,
          vault.id,
        ),
      ...
      if (restoredVault) {
        expect(remainingVault).toEqual(
          expect.objectContaining({ lifecycle: "ACTIVE", deletedAt: null, purgeAfter: null }),
        );
...
async function createSharedVault(ownerId: string, viewerId?: string) {
  const vault = await prisma.vault.create({
    data: {
      ...
      members: {
        create: [
          { userId: ownerId, role: "OWNER" as const, encryptedVaultKey: bytes("owner-key"), keyVersion: 1 },
          ...(viewerId
            ? [{ userId: viewerId, role: "VIEWER" as const, encryptedVaultKey: bytes("viewer-key"), keyVersion: 1 }]
            : []),
        ],
      },
    },
...
function createAccount(vaultId: string, purgeAfter: Date | null) {
  return prisma.authenticatorAccount.create({
    data: {
      vaultId,
      encryptedPayload: bytes("encrypted-account"),
      encryptionVersion: 1,
      deletedAt: purgeAfter ? new Date(purgeAfter.getTime() - 30 * 24 * 60 * 60 * 1000) : null,
      purgeAfter,
    },
  });
}
