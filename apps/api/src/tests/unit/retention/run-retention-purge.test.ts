...
import { runRetentionPurge } from "@api/modules/retention/server";
  ...
  it("drains bounded batches in policy order and reports only opaque identifiers", async () => {
    ...
    const accounts = {
      purgeExpired: vi.fn(async () => {
        order.push("accounts");
        return { purgedIds: accountBatches.shift() ?? [] };
      }),
    };
    const vaults = {
      purgeExpiredVaults: vi.fn(async () => {
        order.push("vaults");
        return { purgedIds: vaultBatches.shift() ?? [] };
      }),
    };
    const audit = {
      purgeExpiredAuditEvents: vi.fn(async () => {
        order.push("audit");
  ...
  it("stops at the configured maximum and flags possible backlog without unbounded work", async () => {
    ...
    expect(report).toEqual(
      expect.objectContaining({
        accountBacklogRemaining: true,
        vaultBacklogRemaining: true,
        auditBacklogRemaining: true,
      }),
    );
  });
