...
import { createEncryptedVaultImportHandler } from "@api/route-handlers/vault-imports/route";
import { ApplicationUser } from "@api/modules/identity/domain/application-user";
import type { EncryptedVaultImportRepository } from "@api/modules/vault-archive/server";
...
const user = new ApplicationUser("user-1", "rhasia:passwordless", "owner-1", "owner@example.test", "ACTIVE");

const request = (body: unknown, headers?: Record<string, string>) =>
  new ApiRequest("https://api.example.test/v1/vault-imports", {
    method: "POST",
    body: JSON.stringify(body),
    headers: { "content-type": "application/json", ...headers },
  });

describe("POST /v1/vault-imports contract", () => {
  ...
  it("rejects oversized requests before invoking persistence", async () => {
    const importArchive = vi.fn<EncryptedVaultImportRepository["import"]>();
    const handler = createHandler(importArchive);
    const response = await handler(request({}, { "content-length": String(8 * 1024 * 1024 + 1) }));
...
function createHandler(importArchive: EncryptedVaultImportRepository["import"]) {
  return createEncryptedVaultImportHandler({ authenticate: async () => user, imports: { import: importArchive } });
}
