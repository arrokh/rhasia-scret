...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const legacy = await prisma.externalIdentity.findUnique({
        where: { issuer_subject: { issuer: "supabase", subject: user.supabaseUserId } },
        select: { id: true },
      });
      const canonical = await prisma.externalIdentity.findUnique({
      ...
      if (legacy && !canonical) {
        await prisma.externalIdentity.update({
          where: { id: legacy.id },
          data: { issuer, email: user.email, emailVerifiedAt: new Date() },
        });
    ...
    const remaining = await prisma.applicationUser.count({
      where: { supabaseUserId: { not: null }, externalIdentities: { none: { issuer } } },
    });
    if (remaining !== 0)
      throw new Error(`External identity backfill incomplete: ${remaining} Application Users remain.`);
