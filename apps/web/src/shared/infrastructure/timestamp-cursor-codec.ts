import { DEFAULT_CURSOR_PAGE_SIZE, MAX_CURSOR_PAGE_SIZE, type CursorPageRequest, type TimestampCursor } from "../application/cursor-page";
...
export type ParsedCursorPageRequest =
  | { valid: true; request: CursorPageRequest }
  | { valid: false; error: "invalid_pagination" | "invalid_cursor" };

export function parseTimestampCursorPageRequest(searchParams: URLSearchParams, scope: string, validKey: (key: string) => boolean = () => true): ParsedCursorPageRequest {
  const rawLimit = searchParams.get("limit");
  const limit = rawLimit === null ? DEFAULT_CURSOR_PAGE_SIZE : Number(rawLimit);
  if (!Number.isInteger(limit) || limit < 1 || limit > MAX_CURSOR_PAGE_SIZE) return { valid: false, error: "invalid_pagination" };
...
export function decodeTimestampCursor(value: string | null, scope: string): TimestampCursor | null | undefined {
  ...
  try {
    ...
    if (parsed.version !== 1 || parsed.scope !== scope || typeof parsed.createdAt !== "string" || typeof parsed.key !== "string" || !parsed.key || parsed.key.length > 256) return undefined;
