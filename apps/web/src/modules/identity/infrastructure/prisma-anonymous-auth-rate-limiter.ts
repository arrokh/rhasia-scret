...
export class PrismaAnonymousAuthRateLimiter {
  ...
  private async increment(operation: string, value: string, now: Date): Promise<AnonymousAuthRateLimitResult> {
    ...
    const bucketHash = Buffer.from(createHmac("sha256", this.secret).update(`${operation}:${value}`, "utf8").digest());
    const record = await prisma.anonymousAuthRateLimitWindow.upsert({
