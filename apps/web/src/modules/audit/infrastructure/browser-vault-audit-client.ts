...
export type VaultAuditEvent = { id: string; eventType: RedactedAuditAction; targetId: string | null; actorUserId: string; actorEmail: string; createdAt: string };
...
export async function loadVaultAuditEvents(vaultId: string, filter: VaultAuditFilter = {}, cursor: string | null = null): Promise<VaultAuditPage> {
  ...
  const response = await browserApiClient.getJson<{ events: Array<Omit<VaultAuditEvent, "eventType"> & { eventType: string }>; nextCursor: string | null }>(`/api/vaults/${encodeURIComponent(vaultId)}/audit-events${query}`, { cache: "no-store" });
  return {
    events: response.events
      .filter((event) => (!filter.accountId || event.targetId === filter.accountId) && (!filter.actorUserId || event.actorUserId === filter.actorUserId))
