...
import {
  useCreateEncryptedAuthenticatorAccountMutation,
...
export function LocalVaultCopyPanel({
  ...
  personalVaultKey,
  personalAccounts,
  onPersonalAccountsCopied,
}: {
  personalVaultId: string;
  personalVaultName: string;
  personalVaultKey: Uint8Array;
  personalAccounts: WorkspaceAuthenticatorAccount[];
  ...
  async function copyLocalToPersonal() {
    ...
    try {
      ...
      setStatus({
        tone: "danger",
        text:
          error instanceof BrowserApiError && error.status === 403
            ? t("personalUnavailable")
            : t("partialFailure", { count: copied.length }),
      });
    } finally {
      setBusy(null);
    }
  ...
  async function copyPersonalToLocal() {
    ...
    if (copiedAccountIds.length) {
      try {
        await recordPersonalVaultAccountCopiesToLocal(personalVaultId, copiedAccountIds);
      } catch {
        auditFailed = true;
      }
    }
    ...
    if (!copiedAccountIds.length) {
      ...
      setStatus({
        tone: auditFailed ? "warning" : "danger",
        text: auditFailed
          ? t("partialLocalFailureAuditWarning", { count: copiedAccountIds.length })
          : t("partialLocalFailure", { count: copiedAccountIds.length }),
      });
    } else {
      setStatus({
        tone: auditFailed ? "warning" : "success",
        text: auditFailed
          ? t("personalToLocalSuccessAuditWarning", { count: copiedAccountIds.length })
          : t("personalToLocalSuccess", { count: copiedAccountIds.length }),
      });
    }
    setBusy(null);
  }
  ...
  if (!record)
    return (
      <div className="grid gap-3">
        <StatusBanner tone="info">{t("notCreated")}</StatusBanner>
        <Button variant="outline" asChild>
          <a href="/local">{t("openLocal")}</a>
        </Button>
      </div>
    );
  ...
  return (
    <div className="grid gap-5">
      <StatusBanner tone="info" title={t("unlockedTitle")}>
        {t("unlockedDescription", { name: vault.name })}
      </StatusBanner>
      {status && (
        <StatusBanner tone={status.tone} role={status.tone === "danger" ? "alert" : "status"}>
          {status.text}
        </StatusBanner>
      )}
      <CopyList
        title={t("localAccounts")}
        empty={t("noLocalAccounts")}
        accounts={vault.accounts}
        selected={selectedLocal}
        onToggle={(id) => toggle(setSelectedLocal, id)}
      />
      <Button
        onClick={() => void copyLocalToPersonal()}
        disabled={!selectedLocal.size || busy !== null}
        aria-busy={busy === "local"}
      >
        {busy === "local" ? <LoaderCircle className="animate-spin" /> : <Upload />}
        {t("copyToPersonal")}
      </Button>
      <CopyList
        title={t("personalAccounts")}
        empty={t("noPersonalAccounts")}
        accounts={personalAccounts}
        selected={selectedPersonal}
        onToggle={(id) => toggle(setSelectedPersonal, id)}
      />
      <Button
        variant="outline"
        onClick={() => void copyPersonalToLocal()}
        disabled={!selectedPersonal.size || busy !== null}
        aria-busy={busy === "personal"}
      >
        {busy === "personal" ? <LoaderCircle className="animate-spin" /> : <Copy />}
        {t("copyToLocal")}
      </Button>
...
function LocalVaultUnlockForm({ onUnlock }: { onUnlock: (passphrase: string) => Promise<void> }) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      setError(false);
      try {
        await onUnlock(value.passphrase);
        form.reset();
      } catch {
        setError(true);
      }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="font-bold text-ink-strong">{t("unlockTitle")}</h3>
      ...
      >
        {(field) => (
          <div className="grid gap-2">
            <Label htmlFor="copy-local-vault-passphrase">{t("passphrase")}</Label>
            <PasswordInput
              id="copy-local-vault-passphrase"
              label={t("passphrase")}
              visible={visible}
              ...
              aria-describedby="copy-local-vault-passphrase-error"
              required
            />
            <FormFieldError id="copy-local-vault-passphrase-error" errors={field.state.meta.errors} />
          </div>
        )}
      </form.Field>
...
function CopyList({
  ...
  title: string;
  empty: string;
  accounts: Array<{ id: string; issuer: string; accountName: string }>;
  selected: Set<string>;
      ...
      {accounts.length ? (
        <ul className="grid list-none gap-2 p-0">
          {accounts.map((account) => (
            <li key={account.id} className="flex items-center gap-3 rounded-md border border-border p-3">
              <Checkbox
                id={`copy-${account.id}`}
                checked={selected.has(account.id)}
                onCheckedChange={() => onToggle(account.id)}
              />
              <Label htmlFor={`copy-${account.id}`} className="min-w-0 cursor-pointer">
                <span className="block truncate font-semibold">{account.issuer}</span>
