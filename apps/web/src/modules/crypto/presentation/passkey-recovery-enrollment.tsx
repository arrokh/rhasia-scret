...
export type PasskeyRecoveryEnrollmentErrorCode =
  | "prfRequired"
  | "challengeExpired"
  | "verificationFailed"
  | "unavailable"
...
export function PasskeyRecoveryEnrollment({ userRootKey }: { userRootKey: Uint8Array }) {
  ...
  const [status, setStatus] = useState<"idle" | "enrolling" | "success" | "error" | "removed" | "removal_error">(
    "idle",
  );
  ...
  return (
    <section className="grid gap-4">
      <div className="grid justify-items-center gap-3 text-center">
        <span
          className={`grid size-14 place-items-center rounded-xl ${enrolled ? "bg-success-surface text-success" : "bg-gold-soft text-ink-strong"}`}
        >
      ...
      {recoveryStatus.isPending ? (
        ...
        <>
          <StatusBanner tone="danger" role="alert" title={t("statusErrorTitle")}>
            {t("statusError")}
          </StatusBanner>
          <Button variant="outline" type="button" onClick={() => void recoveryStatus.refetch()}>
            <RefreshCw />
            {t("retry")}
          </Button>
        </>
        ...
        <>
          <StatusBanner tone="success" title={t("activeTitle")}>
            {t("active")}
          </StatusBanner>
          <p className="text-sm leading-6 text-muted-foreground">{t("activeHelp")}</p>
          <Button variant="outline" asChild>
            <Link href="/vaults/recovery">{t("openRecovery")}</Link>
          </Button>
          <Button
            variant="destructive"
            type="button"
      ...
      {status === "error" && (
        <StatusBanner tone="danger" role="alert" title={t("enrollErrorTitle")}>
          {t(`errors.${errorCode}`)}
        </StatusBanner>
      )}
      {status === "removal_error" && (
        <StatusBanner tone="danger" role="alert" title={t("removeErrorTitle")}>
          {t("removeError")}
        </StatusBanner>
      )}
      {confirmingRemoval && (
        <ConfirmationDialog
          ...
          danger
          pending={removeRecovery.isPending}
