...
export function RememberedBrowserEnrollment({
  profileId,
  userRootKey,
}: {
  profileId: string;
  ...
  const [status, setStatus] = useState<
    "checking" | "idle" | "enrolled" | "removed" | "unsupported" | "cancelled" | "offline" | "error"
  >("checking");
  ...
  return (
    <div className={`grid gap-3 border-t border-border pt-4 ${enrolledAt ? "" : "justify-items-center text-center"}`}>
      <div>
      ...
      {enrolledAt && (
        <StatusBanner tone="success">{t("enrolledAt", { date: formatLocalDateTime(enrolledAt, locale) })}</StatusBanner>
      )}
      <form
          ...
          {(pending) => (
            <>
              <Button
                type="submit"
                variant="outline"
                disabled={!online || pending || removing || status === "checking"}
                aria-busy={pending}
              >
                <Fingerprint />
                {pending ? t("verifying") : enrolledAt ? t("update") : t("remember")}
              </Button>
              {enrolledAt && (
                <Button
                  type="button"
