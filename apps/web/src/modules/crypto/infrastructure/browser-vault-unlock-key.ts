...
export class BrowserArgon2idPort implements KeyDerivationPort {
  deriveArgon2id(
    ...
    return measureBrowserOperation("rhsia:unlock:kdf", () =>
      deriveVaultUnlockKeyMeasured(secret, salt, parameters, signal),
    );
  }
}
...
async function deriveVaultUnlockKeyMeasured(
  secret: string,
  salt: Uint8Array,
  parameters: Argon2idParameters,
  signal?: CancellationPort,
  ...
  if (salt.length !== 16) throw new Error("A 16-byte Vault Unlock salt is required.");
  throwIfCancelled(signal);
  if (typeof Worker === "undefined" || !usesDefaultParameters(parameters))
    return deriveVaultUnlockKeyInline(secret, salt, parameters);
  ...
  try {
    try {
      return await new Promise<Uint8Array>((resolve, reject) => {
        ...
        worker.postMessage({ password: password.buffer, salt: copiedSalt.buffer }, [
          password.buffer,
          copiedSalt.buffer,
        ]);
      });
...
async function deriveVaultUnlockKeyInline(
  secret: string,
  salt: Uint8Array,
  parameters: Argon2idParameters,
): Promise<Uint8Array> {
...
function usesDefaultParameters(parameters: Argon2idParameters): boolean {
  return (
    parameters.memoryKiB === ARGON2_MEMORY_KIB &&
    parameters.iterations === ARGON2_ITERATIONS &&
    parameters.parallelism === ARGON2_PARALLELISM &&
    parameters.outputBytes === VAULT_UNLOCK_KEY_BYTES
  );
}
