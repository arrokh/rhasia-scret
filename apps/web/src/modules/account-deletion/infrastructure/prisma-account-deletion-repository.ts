...
type PrismaDatabase = PrismaClient;
...
export class PrismaAccountDeletionRepository implements AccountDeletionRepository {
  public constructor(
    ...
    private readonly database: PrismaDatabase = prisma,
  ...
  public async createPasswordlessOtpChallenge(
    ...
    const authorizationSeed = randomBytes(32).toString("base64url");
    ...
    const created = await this.database.$transaction(async (transaction) => {
      ...
      return transaction.accountDeletionChallenge.create({
        data: {
          ...
          authorizationDigest: this.digest(AUTHORIZATION_CONTEXT, randomBytes(32).toString("base64url")),
          expiresAt,
        },
        select: { id: true },
      });
  ...
  private digest(context: string, value: string): Uint8Array<ArrayBuffer> {
    return toArrayBuffer(createHmac("sha256", this.secret).update(context).update(value).digest());
  }

  private emailRateLimitBucket(email: string): Uint8Array<ArrayBuffer> {
    return toArrayBuffer(
      createHmac("sha256", this.anonymousAuthSecret).update(`magic-link-email:${email.toLowerCase()}`, "utf8").digest(),
    );
  }
