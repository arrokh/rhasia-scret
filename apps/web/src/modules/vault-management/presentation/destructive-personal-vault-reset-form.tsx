...
export function DestructivePersonalVaultResetForm() {
  ...
  return <form noValidate className="grid gap-5" onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void form.handleSubmit(); }}>
    <StatusBanner tone="danger" title={t("title")} role="alert">
      <p>{t("intro")}</p>
      <ul className="mt-2 list-disc space-y-1 pl-5"><li>{t("account")}</li><li>{t("keys")}</li><li>{t("memberships")}</li><li>{t("services")}</li></ul>
      <p className="mt-2">{t("boundary")}</p>
    </StatusBanner>
    <form.Field name="confirmation" validators={{ onSubmit: ({ value }) => value === DESTRUCTIVE_RESET_CONFIRMATION ? undefined : t("confirmationMismatch") }}>{(field) => <div className="grid gap-2"><Label htmlFor="destructive-reset-confirmation" className="block leading-5">{t.rich("typeToken", { token: () => <strong>{DESTRUCTIVE_RESET_CONFIRMATION}</strong> })}</Label><Input id="destructive-reset-confirmation" value={field.state.value} onChange={(event) => { field.handleChange(event.target.value); if (status !== "resetting") setStatus("idle"); }} autoComplete="off" aria-invalid={field.state.meta.errors.length > 0} aria-describedby={field.state.meta.errors.length ? "destructive-reset-confirmation-error" : undefined} disabled={status === "resetting"} required /><FormFieldError id="destructive-reset-confirmation-error" errors={field.state.meta.errors} /></div>}</form.Field>
    <form.Subscribe selector={(state) => state.isSubmitting}>{(isSubmitting) => <Button variant="destructive" type="submit" disabled={isSubmitting} aria-busy={isSubmitting}><Trash2 />{isSubmitting ? t("deleting") : t("submit")}</Button>}</form.Subscribe>
    {status === "invalid_confirmation" && <StatusBanner tone="danger" role="alert">{t("invalid")}</StatusBanner>}
    {status === "blocked" && <StatusBanner tone="danger" role="alert">{t("blocked", { count: blockedVaults })}</StatusBanner>}
    {status === "reset_error" && <StatusBanner tone="danger" role="alert">{t("error")}</StatusBanner>}
    {status === "cleanup_error" && <StatusBanner tone="danger" role="alert">{t("cleanupError")}</StatusBanner>}
    {status === "confirming" && <ConfirmationDialog title={t("confirmTitle")} description={t("confirmDescription")} confirmLabel={t("confirm")} danger onCancel={() => setStatus("idle")} onConfirm={() => void confirmReset()} />}
  </form>;
}
