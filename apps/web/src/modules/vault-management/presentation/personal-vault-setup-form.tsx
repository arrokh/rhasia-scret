...
export function PersonalVaultSetupForm() {
  ...
  const form = useForm({
    defaultValues: { vaultName: t("defaultVaultName"), secretMode: "generated" as SecretMode, secret: "", confirmation: "", acknowledged: false },
  ...
  return (
    <form noValidate className="grid gap-5" onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void form.handleSubmit(); }}>
        ...
        {(modeField) => (
          <Field>
            ...
            <RadioGroup value={modeField.state.value} onValueChange={(value) => selectSecretMode(value as SecretMode)} className="grid gap-2">
              <Choice id="generated-secret" value="generated" title={t("generatedTitle")} description={t("generatedDescription")} />
              <Choice id="custom-secret" value="custom" title={t("customTitle")} description={t("customDescription")} />
            </RadioGroup>
            <form.Field name="secret" validators={{ onSubmit: ({ value }) => validateSecret(value, t("passphraseInvalid")) }}>
              {(secretField) => modeField.state.value === "generated" ? (
                <div className="grid gap-3 rounded-lg border border-warning/25 bg-warning-surface p-4">
                  ...
                  <output className="rounded-md border border-warning/20 bg-card p-3 text-center font-mono text-sm leading-6 font-bold text-foreground break-words" aria-label={t("vaultPassphrase")}>{secretField.state.value || t("generating")}</output>
                  <Button variant="outline" type="button" disabled={!secretField.state.value} onClick={regenerateSecret}><RefreshCw aria-hidden="true" />{t("regenerate")}</Button>
                ...
                <Field>
                  ...
                  <PasswordInput id="custom-unlock-secret" label={t("yourPassphrase")} value={secretField.state.value} visible={secretVisible} onChange={(event) => { secretField.handleChange(event.target.value); if (form.state.values.confirmation) form.setFieldValue("confirmation", ""); setStatus("idle"); }} onToggleVisibility={() => setSecretVisible((visible) => !visible)} aria-invalid={secretField.state.meta.errors.length > 0} aria-describedby={secretField.state.meta.errors.length ? "custom-secret-error" : "custom-secret-guidance"} autoComplete="new-password" required />
                  <p id="custom-secret-guidance" className="text-xs leading-5 text-muted-foreground">{t("customGuidance")}</p>
      ...
      <form.Field name="confirmation" validators={{ onChange: ({ value }) => value && value !== form.state.values.secret ? t("confirmationMismatch") : undefined, onSubmit: ({ value }) => value === form.state.values.secret ? undefined : t("confirmationMismatch") }}>
        {(field) => <Field><Label htmlFor="unlock-secret-confirmation">{t("reenter")}</Label><PasswordInput id="unlock-secret-confirmation" label={t("confirmation")} value={field.state.value} visible={confirmationVisible} onChange={(event) => { field.handleChange(event.target.value); setStatus("idle"); }} onToggleVisibility={() => setConfirmationVisible((visible) => !visible)} aria-invalid={field.state.meta.errors.length > 0} aria-describedby={field.state.meta.errors.length ? "confirmation-error" : undefined} autoComplete="new-password" required /><FormFieldError id="confirmation-error" errors={field.state.meta.errors} /></Field>}
      </form.Field>

      <form.Field name="acknowledged" validators={{ onSubmit: ({ value }) => value ? undefined : t("acknowledgementRequired") }}>
        {(field) => <Field><div className="flex items-start gap-3 rounded-md border border-border bg-muted/50 p-3"><Checkbox id="setup-acknowledgement" checked={field.state.value} onCheckedChange={(checked) => field.handleChange(checked === true)} aria-invalid={field.state.meta.errors.length > 0} aria-describedby={field.state.meta.errors.length ? "setup-acknowledgement-error" : undefined} /><Label htmlFor="setup-acknowledgement" className="text-sm leading-5 font-normal">{t("acknowledgement")}</Label></div><FormFieldError id="setup-acknowledgement-error" errors={field.state.meta.errors} /></Field>}
      </form.Field>

      <form.Subscribe selector={(state) => ({ isSubmitting: state.isSubmitting, secret: state.values.secret, mode: state.values.secretMode })}>
        {({ isSubmitting, secret, mode }) => <Button type="submit" className="w-full" disabled={(mode === "generated" && !secret) || isSubmitting} aria-busy={isSubmitting}>{isSubmitting && <LoaderCircle className="animate-spin" />}{isSubmitting ? t("securing") : t("secure")}</Button>}
      </form.Subscribe>
      <form.Subscribe selector={(state) => state.isSubmitting}>{(isSubmitting) => isSubmitting ? <StatusBanner tone="info">{t("creatingKeys")}</StatusBanner> : null}</form.Subscribe>
      {status === "setup_error" && <StatusBanner tone="danger" role="alert">{t("error")}</StatusBanner>}
    </form>
  );
}
...
function validateSecret(secret: string, invalidMessage: string): string | undefined { try { validateVaultUnlockSecret(secret); return undefined; } catch { return invalidMessage; } }
