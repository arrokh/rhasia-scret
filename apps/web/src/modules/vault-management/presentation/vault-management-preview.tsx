...
import { SharedVaultDetails, SharedVaultDirectory } from "./shared-vault-manager";
...
export function VaultManagementPreview() {
  ...
  return <div className="grid gap-5"><SurfaceCard aria-label={t("listLabel")}><SharedVaultDirectory vaults={[vault]} /></SurfaceCard><SurfaceCard aria-label={t("detailLabel")}><SharedVaultDetails vault={vault} ownerEmail="owner@local.invalid" initialDefaultAccountPermissions={{ permissions: { canAddAccounts: false, canEditAccounts: false, canDeleteAccounts: false }, revision: 1 }} onRenamed={() => undefined} onAccountDeleted={async () => undefined} /></SurfaceCard></div>;
}
