...
export function VaultMembershipDefaults({
  ...
  const state =
    initial ??
    (defaults.data?.vaultDefaultAccountPermissions
      ? {
          permissions: defaults.data.vaultDefaultAccountPermissions,
          revision: defaults.data.vaultDefaultAccountPermissionsRevision,
        }
      : undefined);
  if (defaults.isPending && !initial) return <FormLoadingPlaceholder />;
  if (defaults.isError && !initial)
    return (
...
export function VaultMembershipOwnerPanel({
  vault,
  active,
  onAudit,
}: {
...
export function VaultMemberPermissionNotice({ effective }: { effective: EffectiveSharedVaultAccountPermissions }) {
  const t = useTranslations("VaultManagement.permissions");
  return (
    <StatusBanner tone="info" title={t("yourTitle")}>
      <span>{t("yourDescription")}</span>
      <span className="mt-2 block">
        <PermissionSummary permissions={effective.permissions} />
      </span>
    </StatusBanner>
  );
}

function VaultDefaultPermissionsForm({
  vaultId,
  permissions,
  revision,
}: {
        ...
        <CollapsibleContent>
          <div className="grid gap-3 border-t p-4">
            <div className="grid gap-3 sm:grid-cols-3">
              {permissionFields.map(({ name, label, description }) => (
                <form.Field key={name} name={name}>
                  {(field) => (
                    <label
                      className="grid grid-cols-[auto_1fr] items-start gap-2 rounded-md bg-card p-3"
                      htmlFor={`vault-default-${name}`}
                    >
                      <Checkbox
                        id={`vault-default-${name}`}
                        checked={field.state.value}
                        onCheckedChange={(checked) => field.handleChange(checked === true)}
                        aria-describedby={`vault-default-${name}-description`}
                      />
                      <span>
                        <span className="block text-sm font-bold">{t(label)}</span>
                        <span
                          id={`vault-default-${name}-description`}
            ...
            >
              {mutation.isPending ? t("saving") : t("saveDefaults")}
            </Button>
            {status && (
              <StatusBanner
                tone={status === "saved" ? "success" : "danger"}
...
function MemberPermissionsDialog({
  ...
  vaultId: string;
  participant: BrowserVaultParticipant;
  onClose: () => void;
}) {
  const t = useTranslations("VaultManagement.permissions");
  ...
  return (
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("memberTitle")}</DialogTitle>
          <DialogDescription>{t("memberDescription", { email: participant.email })}</DialogDescription>
        </DialogHeader>
        <form
          noValidate
        ...
        >
          {permissionFields.map(({ name, label, impact }) => (
            <form.Field key={name} name={name}>
              {(field) => (
                <div className="grid gap-2">
                  ...
                  <p id={`member-permission-${name}-description`} className="text-xs leading-4 text-muted-foreground">
                    {t(impact)}{" "}
                    {t("current", {
                      value: effective.permissions[name] ? t("allowed") : t("denied"),
                      source: t(effective.sources[name] === "MEMBER" ? "memberSource" : "vaultSource"),
                    })}
                  </p>
                </div>
              )}
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={mutation.isPending} aria-busy={mutation.isPending}>
              {mutation.isPending ? t("saving") : t("saveMember")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
...
function overrideValue(value: OverrideChoice): boolean | null {
...
function InvitationPanel({
  ...
  const visibleParticipants = participants.filter(
    ({ invitationId }) => !invitationId || !replacedInvitationIds.includes(invitationId),
  );
  const invited = [
    ...visibleParticipants,
    ...createdInvitations
      .filter(({ id }) => !participants.some((participant) => participant.invitationId === id))
      .map(({ id, email, expiresAt }) => ({
        key: `invitation:${id}`,
        email,
        kind: "INVITATION" as const,
        userId: null,
        invitationId: id,
        invitationState: "PENDING" as const,
  ...
  function rememberInvitation(
    invitation: { id: string; email: string; expiresAt: string; link: string },
    replacedInvitationId?: string,
  ) {
    setPendingLinks((current) => {
      const next = { ...current, [invitation.id]: invitation.link };
      if (replacedInvitationId) delete next[replacedInvitationId];
      return next;
    });
    setCreatedInvitations((current) => [
      ...current.filter(({ id }) => id !== invitation.id && id !== replacedInvitationId),
  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="grid gap-3" aria-labelledby="invited-users-title">
        ...
        {failed && (
          <StatusBanner tone="danger" role="alert">
            {t(invited.length ? "nextError" : "listError")}
          </StatusBanner>
        )}
        {!loading && !failed && !invited.length && (
          <p className="rounded-md border border-dashed bg-muted/30 p-5 text-center text-sm text-muted-foreground">
            {t("empty")}
          </p>
        )}
        {!!invited.length && (
          <ul className="grid list-none gap-2 p-0">
            {invited.map((participant) => {
              ...
              return (
                <li
                  key={participant.key}
                  className="grid items-center gap-3 rounded-md border bg-card p-3 sm:grid-cols-[minmax(0,1fr)_auto]"
                >
                  <span className="grid min-w-0 gap-1">
                    <strong className="break-all text-sm sm:truncate">{participant.email}</strong>
                    <span className="flex flex-wrap items-center gap-2">
                      <Badge
                        className={
                          expired ? "w-fit bg-danger-surface text-destructive" : "w-fit bg-muted text-muted-foreground"
                        }
                      >
                        {participant.kind === "MEMBER" ? t("active") : expired ? t("expired") : t("pending")}
                      </Badge>
                      {participant.effectiveAccountPermissions && (
                        <span className="text-xs text-muted-foreground">
                          <PermissionSummary permissions={participant.effectiveAccountPermissions.permissions} />
                        </span>
                      )}
                    </span>
                  </span>
                  <span className="flex flex-wrap items-center justify-end gap-1">
                    {expired && (
                      <Button
                        ...
                        type="button"
                        disabled={reinvitingInvitationId === invitationId}
                        aria-busy={reinvitingInvitationId === invitationId}
                        onClick={() => void reinvite(participant)}
                      >
                        <RefreshCw className={reinvitingInvitationId === invitationId ? "animate-spin" : undefined} />
                        {reinvitingInvitationId === invitationId ? t("reinviting") : t("reinvite")}
                      </Button>
                    )}
                    {invitationId && !expired && (
                      <Button
                        ...
                        size="icon"
                        type="button"
                        aria-label={t(linkAvailable ? "copyPending" : "copyUnavailableLabel", {
                        ...
                        onClick={() => void copyPendingInvitation(invitationId)}
                      >
                        {linkAvailable ? (
                          copiedInvitationId === invitationId ? (
                            <Check />
                    ...
                    <Button
                      ...
                      size="icon"
                      type="button"
                      aria-label={t("viewAudit", { email: participant.email })}
                      title={participant.userId ? t("viewAuditTitle") : t("auditUnavailable")}
                      disabled={!participant.userId}
                      onClick={() => onAudit(participant)}
                    >
                      <ScrollText />
                    </Button>
                    <Button
                      ...
                      type="button"
                      aria-label={t("deleteLabel", { email: participant.email })}
        ...
        {copyStatus && (
          <StatusBanner
            tone={copyStatus === "failed" ? "danger" : "warning"}
            role={copyStatus === "failed" ? "alert" : "status"}
          >
            {t(copyStatus === "failed" ? "copyError" : "copyUnavailable")}
          </StatusBanner>
        )}
        {hasMore && (
          <Button variant="outline" type="button" disabled={loadingMore} onClick={onLoadMore}>
            {loadingMore ? t("loading") : t("loadMore")}
          </Button>
        )}
        {!!invited.length && !hasMore && (
      ...
      {participantToDelete && (
        <ConfirmationDialog
          title={participantToDelete.kind === "MEMBER" ? t("revokeTitle") : t("deleteTitle")}
          description={
            participantToDelete.kind === "MEMBER"
              ? t("revokeDescription", { email: participantToDelete.email })
              : t("deleteDescription", { email: participantToDelete.email })
          }
          confirmLabel={participantToDelete.kind === "MEMBER" ? t("revoke") : t("delete")}
          danger
      ...
      {participantToConfigure?.userId &&
        participantToConfigure.permissionOverrides &&
        participantToConfigure.effectiveAccountPermissions &&
        participantToConfigure.permissionsRevision && (
          <MemberPermissionsDialog
            key={`${participantToConfigure.userId}:${participantToConfigure.permissionsRevision}`}
            vaultId={vault.id}
            participant={participantToConfigure}
            onClose={() => setParticipantToConfigure(null)}
          />
        )}
    </div>
  );
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <form.Field
          name="email"
          validators={{
            onSubmit: ({ value }) => (/^\S+@\S+\.\S+$/.test(value.trim()) ? undefined : t("invalidEmail")),
          }}
        >
          {(field) => (
            <div className="grid gap-2">
              <Label htmlFor="invitation-email">{t("recipientEmail")}</Label>
              <Input
                id="invitation-email"
                type="email"
                value={field.state.value}
                ...
                aria-describedby={field.state.meta.errors.length ? "invitation-email-error" : undefined}
                required
              />
              <FormFieldError id="invitation-email-error" errors={field.state.meta.errors} />
            </div>
          )}
        </form.Field>
