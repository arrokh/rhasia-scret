  ...
  return (
    <section className="grid min-w-0 gap-5 p-4 sm:p-5" aria-labelledby="account-list-heading">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold tracking-[0.1em] text-muted-foreground uppercase">{t("allVaults")}</p>
        ...
        <span
          ...
          aria-label={t("count", { count: workspace.accounts.length })}
        >
      ...
      {workspace.accounts.length ? (
        ...
        <div className="grid justify-items-center gap-2 rounded-lg border border-dashed border-border bg-muted/40 p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">{current ? t("emptyCurrent") : t("emptySnapshot")}</p>
          {current && (
