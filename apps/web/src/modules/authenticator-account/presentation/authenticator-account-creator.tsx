...
class AccountCreatorError extends Error {
  public constructor(public readonly code: "destinationUnavailable") {
    super(code);
  }
}

export function AuthenticatorAccountCreator({
  personalVaultId,
  preferredVaultId,
}: {
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (
          isDuplicateAccount(
            candidate,
            workspace.accounts.filter((account) => account.vaultId === value.selectedVaultId),
          )
        ) {
  ...
  function openWorkspace(unlocked: UnlockedVaultWorkspace) {
    ...
    setMessage(
      unlocked.unavailableSharedVaults > 0
        ? { source: "creator", key: "unavailableVaults", count: unlocked.unavailableSharedVaults }
        : null,
    );
  }
  ...
  const unlockForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        setMessage({ source: "creator", key: "unlockError" });
      }
    },
  });
  ...
  const renderedMessage =
    message?.source === "totp"
      ? tTotpError(message.key)
      : message?.key === "unavailableVaults"
        ? t(message.key, { count: message.count })
        : message
          ? t(message.key)
          : null;
    ...
    return (
      ...
      >
        ...
        >
          {(field) => (
            <Field>
              <Label htmlFor="account-vault-unlock-secret">{t("passphrase")}</Label>
              <PasswordInput
                id="account-vault-unlock-secret"
                label={t("passphrase")}
                ...
                aria-describedby={field.state.meta.errors.length ? "account-unlock-error" : undefined}
                required
              />
              <FormFieldError id="account-unlock-error" errors={field.state.meta.errors} />
            </Field>
          )}
        </unlockForm.Field>
        <unlockForm.Subscribe selector={(state) => state.isSubmitting}>
          {(isSubmitting) => (
            <Button type="submit" disabled={isSubmitting} aria-busy={isSubmitting}>
              ...
              {isSubmitting ? t("unlocking") : t("continue")}
            </Button>
          )}
    ...
    <>
      ...
      <accountForm.Subscribe selector={(state) => state.values.uri}>
        {(uri) =>
          uri ? (
            <form
              noValidate
              className="grid gap-5 bg-muted/30 p-5 sm:p-6"
              onSubmit={(event) => {
                event.preventDefault();
                event.stopPropagation();
            ...
            >
              ...
              <accountForm.Field
                name="uri"
                validators={{ onSubmit: ({ value }) => (value.trim() ? undefined : t("uriRequired")) }}
              >
                {(field) => (
                  <Field>
                    <Label htmlFor="account-uri">{t("uri")}</Label>
                    <Input
                      id="account-uri"
                      value={field.state.value}
                      ...
                      autoComplete="off"
                      aria-invalid={field.state.meta.errors.length > 0}
                      aria-describedby={field.state.meta.errors.length ? "account-uri-error" : "account-uri-help"}
                      required
                      readOnly
                    />
                    <p id="account-uri-help" className="text-xs text-muted-foreground">
                      {t("uriHelp")}
                    </p>
                    <FormFieldError id="account-uri-error" errors={field.state.meta.errors} />
                  </Field>
                )}
              </accountForm.Field>
              <accountForm.Subscribe selector={(state) => state.values.uri}>
                {(reviewUri) => {
                  ...
                  const details: Array<[string, string | number]> = [
                    [t("issuer"), metadata.issuer],
                    [t("algorithm"), metadata.algorithm],
                    [t("digits"), metadata.digits],
                    [t("period"), t("periodSeconds", { seconds: metadata.period })],
                  ];
                  return (
                    <section
                      className="grid gap-4 rounded-lg border border-warning/25 bg-warning-surface p-4"
                      aria-labelledby="authenticator-metadata-title"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-bold tracking-wider text-warning uppercase">{t("preview")}</p>
                          <h3 id="authenticator-metadata-title" className="mt-1 font-bold text-ink-strong">
                            {t("metadata")}
                          </h3>
                        </div>
                        <Badge className="bg-success-surface text-success">{t("secretDetected")}</Badge>
                      </div>
                      <accountForm.Field
                        name="accountLabel"
                        validators={{ onSubmit: ({ value }) => (value.trim() ? undefined : t("accountLabelRequired")) }}
                      >
                        {(field) => (
                          <Field>
                            ...
                            <Input
                              ...
                              aria-describedby={
                                field.state.meta.errors.length
                                  ? "account-label-help account-label-error"
                                  : "account-label-help"
                              }
                              required
                            />
                            <p id="account-label-help" className="text-xs leading-5 text-muted-foreground">
                              {t("accountLabelHelp")}
                            </p>
                            <FormFieldError id="account-label-error" errors={field.state.meta.errors} />
                          </Field>
                        )}
                      </accountForm.Field>
                      <dl className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                        {details.map(([term, value]) => (
                          <div key={term} className="rounded-md bg-card p-3">
                            <dt className="text-[0.68rem] font-bold text-muted-foreground uppercase">{term}</dt>
      ...
      {duplicate && (
        <div className="m-5 mt-0">
          <StatusBanner tone="warning" title={t("duplicateTitle")}>
            <span>{t("duplicate")}</span>
            <span className="mt-3 flex gap-2">
              <Button size="sm" variant="outline" type="button" onClick={() => setDuplicate(null)}>
                {tCommon("cancel")}
              </Button>
              <Button size="sm" type="button" onClick={() => void saveDuplicate()} disabled={!online}>
                {t("addAnyway")}
              </Button>
            </span>
          </StatusBanner>
...
function parseAuthenticatorMetadata(
  uri: string,
): { issuer: string; accountName: string; algorithm: string; digits: number; period: number } | null {
