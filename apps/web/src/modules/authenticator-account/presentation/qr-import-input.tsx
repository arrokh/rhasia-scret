  ...
  return (
    <section className={cn("grid gap-5 p-5 sm:p-6", className)} aria-labelledby="qr-import-title">
      ...
      <Button
        type="button"
        className="w-full"
        disabled={uploadLoading || cameraLoading}
        aria-busy={cameraLoading}
        onClick={openCamera}
      >
        {cameraLoading ? <LoaderCircle className="animate-spin" /> : <Camera />}
        <span>{cameraLoading ? t("loadingScanner") : t("scanCamera")}</span>
      </Button>
      <Collapsible className="rounded-lg border border-border bg-muted/30">
        <CollapsibleTrigger asChild>
          <Button variant="ghost" type="button" className="w-full justify-between px-4">
            <span>{t("advancedOptions")}</span>
            <ChevronDown className="transition-transform [[data-state=open]_&]:rotate-180" aria-hidden="true" />
          </Button>
        </CollapsibleTrigger>
        <CollapsibleContent className="grid gap-4 border-t border-border p-4">
          <div className="grid gap-2">
            <Button variant="outline" asChild className="w-full" aria-disabled={uploadLoading}>
              <label htmlFor="qr-image">
                {uploadLoading ? <LoaderCircle className="animate-spin" /> : <ImageUp />}
                <span className="max-w-[15rem] truncate">
                  {uploadLoading ? t("loadingScanner") : fileName || t("upload")}
                </span>
              </label>
            </Button>
            <input
          ...
          >
            ...
            >
              {(field) => (
                <div className="grid gap-2">
                  <Label htmlFor="manual-authenticator-uri">{t("manualLabel")}</Label>
                  <Input
                    id="manual-authenticator-uri"
                    ...
                    aria-describedby={
                      field.state.meta.errors.length
                        ? "manual-authenticator-uri-error"
                        : "manual-authenticator-uri-help"
                    }
                    required
                  />
                  <p id="manual-authenticator-uri-help" className="text-xs leading-5 text-muted-foreground">
                    {t("manualHelp")}
                  </p>
                  <FormFieldError id="manual-authenticator-uri-error" errors={field.state.meta.errors} />
                </div>
              )}
            </manualForm.Field>
      ...
      <Dialog open={cameraOpen} onOpenChange={() => undefined}>
        <DialogContent
          className="max-w-lg overflow-hidden bg-card p-4"
        ...
        >
          ...
          <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg bg-ink-strong sm:aspect-video">
            ...
            {cameraReady && (
              <div
                className="pointer-events-none absolute inset-[16%_20%] rounded-lg border-2 border-card shadow-[0_0_0_999px_rgb(23_29_34/35%)]"
                aria-hidden="true"
              >
            ...
            {cameraLoading && (
              <span
                className="absolute inset-0 grid place-items-center bg-ink-strong/50 text-card"
                role="status"
                aria-label={t("loadingScanner")}
              >
