...
export function VaultWorkspaceUnlock({ personalVaultId, onUnlocked }: { personalVaultId: string; onUnlocked: (workspace: UnlockedVaultWorkspace) => void }) {
  ...
  return <form noValidate className="grid gap-5 p-5 sm:p-6" onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void form.handleSubmit(); }}>
    <div className="grid justify-items-center gap-3 text-center">
      ...
      <div><h2 className="text-xl font-bold text-ink-strong">{t("title")}</h2><p className="mt-1 text-sm leading-5 text-muted-foreground">{t("description")}</p></div>
    </div>
    <form.Field name="secret" validators={{ onSubmit: ({ value }) => value.trim() ? undefined : t("passphraseRequired") }}>
      {(field) => <div className="grid gap-2"><Label htmlFor="vault-unlock-secret">{t("passphrase")}</Label><PasswordInput id="vault-unlock-secret" label={t("passphrase")} visible={secretVisible} onToggleVisibility={() => setSecretVisible((visible) => !visible)} value={field.state.value} onChange={(event) => field.handleChange(event.target.value)} aria-invalid={field.state.meta.errors.length > 0} aria-describedby={field.state.meta.errors.length ? "vault-unlock-secret-error" : undefined} disabled={passkeyUnlocking || rememberedUnlocking} required autoComplete="current-password" /><FormFieldError id="vault-unlock-secret-error" errors={field.state.meta.errors} /></div>}
    </form.Field>
    <form.Subscribe selector={(state) => state.isSubmitting}>{(isSubmitting) => <Button type="submit" className="w-full" disabled={isSubmitting || passkeyUnlocking || rememberedUnlocking} aria-busy={isSubmitting}>{isSubmitting ? t("unlocking") : t("unlock")}</Button>}</form.Subscribe>
