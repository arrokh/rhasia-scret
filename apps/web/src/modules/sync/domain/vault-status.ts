...
export type VaultStatusKind = "DEVICE_ONLY" | "CURRENT" | "OFFLINE_SNAPSHOT" | "RECONNECTING" | "AUTH_REQUIRED" | "STALE" | "ERROR" | "LOCAL_STORAGE_ERROR";
...
export function resolveVaultStatus(input: VaultStatusInput): VaultStatus {
  if (input.origin === "LOCAL") return { kind: "DEVICE_ONLY", origin: "LOCAL", capability: "LOCAL_WRITABLE", onlineHint: input.onlineHint };
  const syncState = input.syncState ?? "ERROR";
  const kind: VaultStatusKind = syncState === "CURRENT"
    ? "CURRENT"
    : syncState === "OFFLINE"
      ? "OFFLINE_SNAPSHOT"
      : syncState === "SYNCING"
        ...
        : syncState === "AUTH_REQUIRED"
          ...
          : syncState === "ERROR"
            ...
            : syncState === "LOCAL_STORAGE_ERROR"
              ? "LOCAL_STORAGE_ERROR"
              : "STALE";
  return { kind, origin: input.origin, capability: kind === "CURRENT" ? "SERVER_WRITABLE" : "READ_ONLY", onlineHint: input.onlineHint, lastSynchronizedAt: input.lastSynchronizedAt };
}
