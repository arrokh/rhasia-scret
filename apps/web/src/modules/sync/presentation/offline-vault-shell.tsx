...
export function OfflineVaultShell() {
  ...
  async function unlockRemembered() {
    ...
    setStatus("ready");
    try {
      replaceWorkspace(await loadOfflineVaultWorkspaceWithRememberedBrowser(profileId));
      captureAnalyticsEvent(ANALYTICS_EVENTS.offlineVaultUnlocked, { method: "remembered_browser" });
    } catch {
      captureAnalyticsEvent(ANALYTICS_EVENTS.offlineVaultUnlockFailed, {
  ...
  if (workspace)
    return (
      <UnlockedOfflineWorkspace
        workspace={workspace}
        onLock={() => {
          replaceWorkspace(null);
          captureAnalyticsEvent(ANALYTICS_EVENTS.offlineVaultLocked);
        }}
        onClear={() => void clearDevice()}
      />
    );
  ...
  return (
    <AppPage>
      ...
      <SurfaceCard className="grid gap-5 p-5 sm:p-6">
        ...
        <StatusBanner tone="offline" title={t("readOnly")}>
        ...
        {profiles.length > 0 && (
          ...
          >
            <form.Field name="profileId">
              {(field) => (
                <div className="grid gap-2">
                  <Label htmlFor="offline-profile">{t("profile")}</Label>
                  <Select value={field.state.value} onValueChange={field.handleChange}>
                    <SelectTrigger id="offline-profile" className="w-full">
            ...
            >
              {(field) => (
                <div className="grid gap-2">
                  <Label htmlFor="offline-secret">{t("passphrase")}</Label>
                  <PasswordInput
                    id="offline-secret"
                    label={t("passphrase")}
                    visible={secretVisible}
                    onToggleVisibility={() => setSecretVisible((value) => !value)}
                    value={field.state.value}
                    onChange={(event) => field.handleChange(event.target.value)}
                    aria-invalid={field.state.meta.errors.length > 0}
                    aria-describedby={field.state.meta.errors.length ? "offline-secret-error" : undefined}
                    autoComplete="current-password"
                    required
                  />
                  <FormFieldError id="offline-secret-error" errors={field.state.meta.errors} />
                </div>
              )}
            </form.Field>
            <form.Subscribe selector={(state) => state.isSubmitting}>
              {(pending) => (
                <Button type="submit" disabled={pending} aria-busy={pending}>
                  <KeyRound />
                  {pending ? t("unlocking") : t("unlockPassphrase")}
                </Button>
              )}
            </form.Subscribe>
            <Button type="button" variant="outline" onClick={() => void unlockRemembered()}>
              <Fingerprint />
              {t("unlockLocal")}
            </Button>
          </form>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" asChild>
            <Link href="/sign-in">{t("backSignIn")}</Link>
          </Button>
          {profiles.length > 0 && (
            <Button type="button" variant="ghost" className="text-destructive" onClick={() => void clearDevice()}>
              <Trash2 />
              {t("clearDevice")}
            </Button>
          )}
      ...
      <SurfaceCard className="grid gap-5 p-4 sm:p-5">
        ...
        {workspace.syncState === "CURRENT" ? (
          <p className="text-sm text-muted-foreground">
            {t("syncComplete")}{" "}
            <Link href="/vaults" className="font-bold underline">
              {t("continueOnline")}
            </Link>
            .
          </p>
        ) : (
          <p className="text-sm text-muted-foreground">{t("staleDescription")}</p>
        )}
        ...
        <Button type="button" variant="ghost" className="justify-self-start text-destructive" onClick={onClear}>
          <Trash2 />
          {t("clearSnapshot")}
        </Button>
      </SurfaceCard>
