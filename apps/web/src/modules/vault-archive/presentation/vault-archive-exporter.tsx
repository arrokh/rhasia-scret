...
export function VaultArchiveExporter({ workspace }: { workspace: UnlockedVaultWorkspace }) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      clearResult();
      if (!online) {
        setMessage("offlineUnavailable");
        return;
      }
      const vault = ownedVaults.find(({ id }) => id === value.vaultId);
      if (!vault) {
        setMessage("vaultUnavailable");
        return;
      }
      let next: PreparedVaultArchive | null = null;
      try {
        next = await prepareEncryptedVaultArchive(
          vault,
          workspace.accounts.filter((account) => account.vaultId === vault.id),
        );
  ...
  async function copyKey() {
    if (!prepared) return;
    try {
      await browserClipboard.writeText(prepared.keyMaterial);
      setCopied(true);
    } catch {
  ...
  if (prepared)
    return (
      <div className="grid gap-5 p-5 sm:p-6">
        ...
        <StatusBanner tone="warning" title={t("bothRequiredTitle")}>
          {t("bothRequiredDescription")}
        </StatusBanner>
        <div className="grid gap-2">
          <Label htmlFor="generated-archive-key">{t("archiveKey")}</Label>
          <PasswordInput
            id="generated-archive-key"
            label={t("archiveKey")}
            visible={keyVisible}
            onToggleVisibility={() => setKeyVisible((value) => !value)}
            value={prepared.keyMaterial}
            readOnly
            autoComplete="off"
          />
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          <Button variant="outline" type="button" onClick={() => void copyKey()}>
            {copied ? <Check /> : <Clipboard />}
            {copied ? t("keyCopied") : t("copyKey")}
          </Button>
          <Button variant="outline" type="button" onClick={downloadKey}>
            <Download />
            {t("downloadKey")}
          </Button>
        </div>
  ...
  return (
    <div className="grid gap-5 p-5 sm:p-6">
      ...
      >
        ...
        <form.Field name="vaultId" validators={{ onSubmit: ({ value }) => (value ? undefined : t("vaultRequired")) }}>
          {(field) => (
            <div className="grid gap-2">
              <Label htmlFor="archive-export-vault">{t("vault")}</Label>
              <Select value={field.state.value} onValueChange={field.handleChange}>
                ...
                >
                  <SelectValue placeholder={t("chooseVault")} />
                </SelectTrigger>
                <SelectContent>
                  {ownedVaults.map((vault) => (
                    <SelectItem key={vault.id} value={vault.id}>
                      {t("vaultOption", {
                        name: vault.name,
                        count: workspace.accounts.filter((account) => account.vaultId === vault.id).length,
                      })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <FormFieldError id="archive-export-vault-error" errors={field.state.meta.errors} />
            </div>
          )}
        </form.Field>
        <form.Field
          ...
          {(field) => (
            <div className="grid gap-2">
              <div className="flex items-start gap-3">
                ...
                />
                <Label htmlFor="archive-key-acknowledgement" className="leading-5">
                  {t("acknowledgement")}
                </Label>
              </div>
              <FormFieldError id="archive-key-acknowledgement-error" errors={field.state.meta.errors} />
            </div>
          )}
        </form.Field>
