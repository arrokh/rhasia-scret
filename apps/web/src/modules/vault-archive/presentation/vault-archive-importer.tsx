...
import {
  uploadEncryptedVaultImport,
  VaultImportClientError,
  type BrowserEncryptedVaultImportRequest,
...
type ArchiveErrorKey =
  ...
  | "locked"
  | "newVaultMaterialUnavailable"
  | "responseMismatch"
  | "refreshError"
  | "partialFailure"
  | "invalidKeyLength"
  | "invalidKey"
  | "clientDestinationUnavailable"
  | "clientConflict"
  | "clientInvalidPayload"
  | "clientTimeout"
  | "clientServerError";

class VaultArchivePresentationError extends Error {
  public constructor(public readonly code: ArchiveErrorKey) {
    super(code);
  }
}
...
export function VaultArchiveImportWorkspace({ personalVaultId }: { personalVaultId: string }) {
  ...
  if (workspace.syncState !== "CURRENT")
    return (
      <div className="grid gap-4 p-5 sm:p-6">
        <StatusBanner tone="offline">{t("blocked")}</StatusBanner>
...
export function VaultArchiveImporter({
  ...
  const previewForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      setSuccess(null);
      if (!online) {
        setErrorCode("offlineOpen");
        return;
      }
      if (!value.archive) {
        setErrorCode("chooseArchive");
        return;
      }
      let archiveBytes: Uint8Array | undefined;
      let archiveKey: Uint8Array | undefined;
      try {
        if (value.archive.size === 0 || value.archive.size > MAX_ENCRYPTED_VAULT_ARCHIVE_BYTES)
          throw new VaultArchivePresentationError("archiveTooLarge");
  ...
  const duplicateCount = useMemo(() => {
    if (!opened) return 0;
    const destinationAccounts = existingDestination
      ? workspace.accounts.filter(({ vaultId }) => vaultId === existingDestination.id)
      : [];
  ...
  function replaceOpened(next: OpenedVaultArchive | null) {
    setOpenedState((current) => {
      if (current && current !== next) clearOpenedVaultArchive(current);
      return next;
    });
  ...
  async function submitImport(destinationId: string, allowDuplicates: boolean) {
    ...
    if (!destination && destinationId !== NEW_SHARED_DESTINATION) {
      setErrorCode("destinationUnavailable");
      return;
    }
    if (duplicateCount > 0 && !allowDuplicates) {
      setDuplicateConfirmation(true);
      return;
    }
    setImportPhase("preparing");
    const plan =
      planRef.current?.selection === destinationId && planRef.current.accountIds.length === opened.accounts.length
        ? planRef.current
        : {
            selection: destinationId,
            vaultId: destination ? destination.id : crypto.randomUUID(),
            accountIds: opened.accounts.map(() => crypto.randomUUID()),
          };
    ...
    try {
      ...
      const request: BrowserEncryptedVaultImportRequest = {
        ...
        accounts: encryptedAccounts.map((encryptedPayload, index) => ({
          id: plan.accountIds[index],
      ...
      if (result.vaultId !== plan.vaultId || result.accountIds.join(",") !== plan.accountIds.join(","))
        throw new VaultArchivePresentationError("responseMismatch");
      uploaded = true;
      setImportPhase("refreshing");
      const refreshed = await refreshAfterImport(workspace);
      if (refreshed) replaceWorkspace(refreshed);
      replaceOpened(null);
      captureAnalyticsEvent(ANALYTICS_EVENTS.vaultArchiveImportCompleted, {
        account_count: plan.accountIds.length,
        destination_type: destination?.type ?? "SHARED",
        created_new_vault: result.vaultCreated,
      });
      setSuccess({
        count: plan.accountIds.length,
        newVault: result.vaultCreated,
        vaultId: result.vaultId,
        vaultType: destination?.type ?? "SHARED",
      });
    } catch (error) {
      if (!activeRef.current) return;
      ...
      <div className="grid gap-5 p-5 sm:p-6">
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            void previewForm.handleSubmit();
          }}
        >
          <SectionHeading icon={ArchiveRestore} title={t("openTitle")} description={t("openDescription")} />
          <previewForm.Field
            name="archive"
            validators={{ onSubmit: ({ value }) => (value ? undefined : t("archiveRequired")) }}
          >
            {(field) => (
              <Field>
                <Label htmlFor="vault-archive-file">{t("archiveFile")}</Label>
                <Input
                  ref={fileInputRef}
                  id="vault-archive-file"
                  type="file"
                  accept=".rhasia-vault,application/octet-stream"
                  onChange={(event) => field.handleChange(event.target.files?.[0] ?? null)}
                  aria-invalid={field.state.meta.errors.length > 0}
                  aria-describedby={
                    field.state.meta.errors.length
                      ? "vault-archive-file-help vault-archive-file-error"
                      : "vault-archive-file-help"
                  }
                />
                <p id="vault-archive-file-help" className="text-xs text-muted-foreground">
                  {t("archiveHelp")}
                </p>
                <FormFieldError id="vault-archive-file-error" errors={field.state.meta.errors} />
              </Field>
            )}
          </previewForm.Field>
          <previewForm.Field
            name="keyMaterial"
            validators={{ onSubmit: ({ value }) => (value.trim() ? undefined : t("keyRequired")) }}
          >
            {(field) => (
              <Field>
                <Label htmlFor="vault-archive-key">{t("keyLabel")}</Label>
                <PasswordInput
                  id="vault-archive-key"
                  label={t("key")}
                  visible={keyVisible}
                  onToggleVisibility={() => setKeyVisible((visible) => !visible)}
                  value={field.state.value}
                  onChange={(event) => field.handleChange(event.target.value)}
                  autoComplete="off"
                  aria-invalid={field.state.meta.errors.length > 0}
                  aria-describedby={
                    field.state.meta.errors.length ? "vault-archive-key-error" : "vault-archive-key-help"
                  }
                />
                <p id="vault-archive-key-help" className="text-xs text-muted-foreground">
                  {t("keyHelp")}
                </p>
                <FormFieldError id="vault-archive-key-error" errors={field.state.meta.errors} />
              </Field>
            )}
          </previewForm.Field>
  ...
  return (
    <div className="grid gap-5 p-5 sm:p-6">
      ...
      <dl className="grid grid-cols-2 gap-3 rounded-lg border bg-muted/30 p-4">
        <div>
          <dt className="text-xs font-bold text-muted-foreground uppercase">{t("vaultName")}</dt>
          <dd className="mt-1 break-words font-bold">{opened.vaultName}</dd>
        </div>
        <div>
          <dt className="text-xs font-bold text-muted-foreground uppercase">{t("accountCount")}</dt>
          <dd className="mt-1 font-bold">{opened.accounts.length}</dd>
        </div>
      </dl>
      <form
        noValidate
      ...
      >
        <importForm.Field
          name="destinationId"
          validators={{ onSubmit: ({ value }) => (value.trim() ? undefined : t("destinationRequired")) }}
        >
          {(field) => (
            <Field>
              <Label htmlFor="archive-import-destination">{t("destination")}</Label>
              <Select value={field.state.value} onValueChange={changeDestination}>
                <SelectTrigger
                  id="archive-import-destination"
                  className="h-12 w-full"
                  aria-invalid={field.state.meta.errors.length > 0}
                  aria-describedby={field.state.meta.errors.length ? "archive-import-destination-error" : undefined}
                >
                ...
                <SelectContent>
                  ...
                  <SelectItem value={NEW_SHARED_DESTINATION}>{t("newShared", { name: opened.vaultName })}</SelectItem>
                </SelectContent>
              </Select>
              <FormFieldError id="archive-import-destination-error" errors={field.state.meta.errors} />
            </Field>
          )}
        </importForm.Field>
        {duplicateCount > 0 && (
          <StatusBanner tone="warning" title={t("duplicatesTitle", { count: duplicateCount })}>
            {t("duplicatesDescription")}
          </StatusBanner>
        )}
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <Button type="button" variant="outline" onClick={cancelImport} disabled={uploading}>
            {tCommon("cancel")}
          </Button>
          <Button type="submit" disabled={!online || uploading} aria-busy={uploading}>
            {uploading && <LoaderCircle className="animate-spin" />}
            {uploading ? t("importing") : t("confirmImport")}
          </Button>
        </div>
      </form>
      {importProgress && (
        <StatusBanner tone="info" role="status" title={importProgress.title}>
          {importProgress.description}
        </StatusBanner>
      )}
      {duplicateConfirmation && (
        <StatusBanner tone="warning" title={t("duplicateConfirmTitle")}>
          <span className="grid gap-3">
            <span>{t("duplicateConfirmDescription")}</span>
            <span className="flex gap-2">
              <Button size="sm" variant="outline" type="button" onClick={cancelImport}>
                {tCommon("cancel")}
              </Button>
              <Button
                size="sm"
                ...
                onClick={() => void submitImport(selectedDestination, true)}
              >
                {t("addAnyway")}
              </Button>
            </span>
      ...
      {errorCode && (
        <StatusBanner tone="danger" role="alert" title={t("notCompleted")}>
...
function ImportSuccessDialog({
  ...
  const destinationHref =
    success.vaultType === "PERSONAL"
      ? "/vaults/manage/personal"
      : `/vaults/manage/${encodeURIComponent(success.vaultId)}`;
  return (
    <Dialog
      open
      onOpenChange={(open) => {
        if (!open) onImportAnother();
      }}
    >
      <DialogContent className="max-w-sm rounded-lg border-border bg-card p-5 shadow-sheet">
        <DialogHeader className="items-center text-center sm:items-center sm:text-center">
          <span
            className="grid size-12 place-items-center rounded-full bg-success-surface text-success"
            aria-hidden="true"
          >
        ...
        <DialogFooter className="-mx-5 -mb-5 mt-1 grid gap-2 bg-muted/60 p-4 sm:grid-cols-2">
          <Button
            variant="outline"
            className="h-auto min-h-12 py-3 text-center leading-5 whitespace-normal"
            type="button"
            onClick={onImportAnother}
          >
            {t("importAnother")}
          </Button>
          <Button className="h-auto min-h-12 py-3 text-center leading-5 whitespace-normal" asChild>
            <Link href={destinationHref}>{t("goToImportedVault")}</Link>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function classifyArchiveError(error: unknown, fallback: ArchiveErrorKey): ArchiveErrorKey {
  if (
    error instanceof VaultArchivePresentationError ||
    error instanceof VaultArchiveWorkflowError ||
...
function parseArchiveKey(value: string): Uint8Array {
  ...
  if (!/^(?:[A-Za-z0-9+/]{4}){10}[A-Za-z0-9+/]{3}=$/.test(normalized))
    throw new VaultArchivePresentationError("invalidKeyLength");
  let key: Uint8Array;
  try {
    key = base64ToBytes(normalized);
  } catch {
    throw new VaultArchivePresentationError("invalidKey");
  }
  if (key.length !== 32) {
    key.fill(0);
    throw new VaultArchivePresentationError("invalidKeyLength");
