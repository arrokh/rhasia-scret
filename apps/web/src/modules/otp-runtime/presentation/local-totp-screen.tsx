...
export function LocalTotpScreen() {
  ...
  const seconds =
    configuration && counter !== null
      ? Math.max(0, Math.ceil(((counter + 1) * configuration.period * 1_000 - now) / 1_000))
      : 0;
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      try {
        ...
        setCode("");
        setError(reason instanceof TotpConfigurationError ? reason.code : "invalidUri");
      }
    },
  });
  ...
  useEffect(() => {
    ...
    generateTotp(configuration, new BrowserHmacGenerator(), new Date(counter * configuration.period * 1_000))
      .then((next) => {
        if (!cancelled)
          setCode((current) => {
            if (current !== next.value) setCopied(false);
            return next.value;
          });
      })
      .catch(() => {
        if (!cancelled) setError("generation");
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <AppPage>
      ...
      <SurfaceCard className="grid gap-6 p-5 sm:p-7">
        <form
          noValidate
          ...
          >
            {(field) => (
              <div className="grid gap-2">
                <Label htmlFor="totp-uri">{t("uri")}</Label>
                <Input
                  id="totp-uri"
                  ...
                  aria-describedby={field.state.meta.errors.length ? "totp-uri-error" : undefined}
                  required
                />
                <FormFieldError id="totp-uri-error" errors={field.state.meta.errors} />
              </div>
            )}
          </form.Field>
        ...
        {error && (
          <StatusBanner tone="danger" role="alert">
            {error === "generation" ? t("generationError") : error === "copy" ? t("copyError") : tError(error)}
          </StatusBanner>
        )}
        {clockDriftWarning && (
        ...
        {configuration && (
          <section className="grid justify-items-center gap-3 rounded-lg border bg-muted/40 p-5">
            <div className="text-center">
              <h2 id="totp-title" className="font-bold text-ink-strong">
                {configuration.issuer}
              </h2>
              <p className="text-sm text-muted-foreground">{configuration.accountName}</p>
            </div>
            <output className="font-mono text-4xl leading-10 font-semibold text-ink-strong" aria-label={t("current")}>
              {formatOtp(code)}
            </output>
            <p className="text-sm text-muted-foreground">{t("remaining", { seconds })}</p>
            <Button className="w-full" type="button" onClick={() => void copyCode()}>
              <Copy />
