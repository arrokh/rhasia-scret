...
test.describe("production Remembered Browser UI", () => {
  ...
  test("enrolls explicitly with required user verification, stores ciphertext only, and removes local material", async ({
    ...
    expect(
      (calls.find(({ operation }) => operation === "create")?.options as PublicKeyCredentialCreationOptions)
        .authenticatorSelection?.userVerification,
    ).toBe("required");
    expect(
      (calls.find(({ operation }) => operation === "get")?.options as PublicKeyCredentialRequestOptions)
        .userVerification,
    ).toBe("required");
  ...
  test("a copied browser record or credential identifier cannot unlock until PRF verification succeeds", async ({
    page,
  }) => {
    ...
    for (const sensitive of [base64(userRootKey), base64(personalVaultKey), vaultUnlockSecret])
      expect(persisted.allPersistentText).not.toContain(sensitive);
    await page.reload();
    await expect(page.getByRole("button", { name: "Buka dengan Verifikasi Lokal" })).toBeVisible();

    await page.evaluate(() => {
      (window as unknown as { __webauthnMode: string }).__webauthnMode = "missing-prf";
    });
    await page.getByRole("button", { name: "Buka dengan Verifikasi Lokal" }).click();
    await expect(
      page.getByRole("alert").filter({ hasText: "tidak ada kunci yang dilepas tanpa verifikasi" }),
    ).toBeVisible();
    await expect(page.getByText("Unlocked Vault Session berhasil dibuat.")).toHaveCount(0);
    await expect(page.getByRole("textbox", { name: "Passphrase Brankas" })).toBeVisible();

    await page.evaluate(() => {
      (window as unknown as { __webauthnMode: string }).__webauthnMode = "valid";
    });
    await page.getByRole("button", { name: "Buka dengan Verifikasi Lokal" }).click();
      ...
      Object.defineProperty(navigator, "credentials", {
        ...
        value: {
          create: async ({ publicKey }: { publicKey: unknown }) => {
            ...
            return (window as unknown as { __webauthnMode: string }).__webauthnMode === "cancel-create"
              ? null
              : new FakeCredential();
          },
          get: async ({ publicKey }: { publicKey: unknown }) => {
            calls.push({ operation: "get", options: publicKey });
            if ((window as unknown as { __webauthnMode: string }).__webauthnMode === "invalid-credential")
              throw new DOMException("Invalid credential", "NotAllowedError");
  ...
  return {
    ...
    cryptoProfile: {
      ...
      wrappedUserRootKey: base64(
        await envelope(derived, userRootKey, {
      ...
      encryptedPersonalVaultKey: base64(
        await envelope(userRootKey, personalVaultKey, {
    ...
    personalVault: {
      ...
      encryptedName: base64(
        await envelope(personalVaultKey, new TextEncoder().encode("Remembered Preview Vault"), {
          purpose: "vault-name",
    ...
    async (value) =>
      new Promise<void>((resolve, reject) => {
        ...
        open.onupgradeneeded = () => {
          if (!open.result.objectStoreNames.contains("encrypted-snapshots"))
            open.result.createObjectStore("encrypted-snapshots", { keyPath: "profileId" });
          if (!open.result.objectStoreNames.contains("remembered-browsers"))
            open.result.createObjectStore("remembered-browsers", { keyPath: "profileId" });
        };
  ...
  return page.evaluate(async () => {
    const stored = await new Promise<{ remembered: unknown[]; snapshots: unknown[] }>((resolve, reject) => {
      ...
      open.onsuccess = () => {
        ...
        transaction.oncomplete = () => {
          open.result.close();
          resolve({ remembered: remembered.result as unknown[], snapshots: snapshots.result as unknown[] });
        };
        transaction.onerror = () => reject(transaction.error);
      };
    });
    ...
    return {
      packageText,
      allPersistentText: JSON.stringify({
        ...stored,
