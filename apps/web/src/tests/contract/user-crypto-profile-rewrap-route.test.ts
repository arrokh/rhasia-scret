...
describe("POST /api/user-crypto-profile/rewrap contract", () => {
  it("updates only opaque rewrapped User Root Key material for an active user", async () => {
    ...
    const response = await handler(
      new NextRequest("http://localhost/api/user-crypto-profile/rewrap", {
        method: "POST",
  ...
  it("accepts the migrated Personal Vault Encryption Key with the rewrapped root key", async () => {
    ...
    const response = await handler(
      new NextRequest("http://localhost/api/user-crypto-profile/rewrap", {
        ...
        body: JSON.stringify({
          ...payload,
          encryptedPersonalVaultKey: Buffer.from("new-personal-vault-key-material").toString("base64"),
        }),
      }),
    );
    expect(response.status).toBe(204);
    expect(rewrapUserRootKey).toHaveBeenCalledWith(
      "user-1",
      expect.objectContaining({ encryptedPersonalVaultKey: expect.any(Buffer), encryptionVersion: 1 }),
    );
  });
});
