...
const payload = {
  encryptedName: Buffer.from("encrypted-shared-vault-name").toString("base64"),
  encryptedOwnerVaultKey: Buffer.from("encrypted-owner-vault-key").toString("base64"),
  encryptionVersion: 1,
};
...
describe("PATCH /api/shared-vaults/:vaultId contract", () => {
  it("lets an owner replace only the encrypted Vault Name", async () => {
    ...
    const response = await handler(
      new NextRequest("http://localhost/api/shared-vaults/vault-1", {
        method: "PATCH",
...
describe("POST /api/shared-vaults contract", () => {
  it("authenticates before consulting the application-user limiter", async () => {
    ...
    const response = await handler(
      new NextRequest("http://localhost/api/shared-vaults", { method: "POST", body: "invalid" }),
    );
  ...
  it("creates a Shared Vault from opaque owner material", async () => {
    ...
    const response = await handler(
      new NextRequest("http://localhost/api/shared-vaults", { method: "POST", body: JSON.stringify(payload) }),
    );
