...
describe("Personal Vault accounts API", () => {
  ...
  it("creates only opaque encrypted content for an active owner", async () => {
    ...
    const response = await handlers.POST(
      new NextRequest("http://localhost/api/vaults/vault-1/accounts", {
        method: "POST",
  ...
  it("does not list accounts for an unauthenticated caller", async () => {
    const handlers = createPersonalAccountsHandlers({
      ...authenticationDependencies(new FakeSessionVerifier(null), {
        provision: async () => {
    ...
    const response = await handlers.GET(new NextRequest("http://localhost/api/vaults/vault-1/accounts"), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
