...
import { createAuthenticatedApplicationExecutor } from "@/modules/server-composition";
...
describe("Personal Vault accounts API", () => {
  function authenticationDependencies(
    ...
    applicationUsers: { provision: (principal: never) => Promise<ApplicationUser> },
  ) {
    const execute = createAuthenticatedApplicationExecutor({
      verifySession: (assurance) => sessionVerifier.verify(assurance),
      provisionApplicationUser: (principal) => applicationUsers.provision(principal as never),
      checkApplicationRateLimit: async () => ({ status: "allowed" as const }),
    });
    return {
      authenticateReader: async (assurance: "fresh-provider-user") => {
        const result = await execute({ assurance, access: "reader" });
        return result.status === "allowed" ? result.user : authenticatedApplicationFailureResponse(result);
      },
      authenticateMutation: async (operation: "account_mutation", assurance: "fresh-provider-user") => {
        const result = await execute({ assurance, access: "mutation", operation });
        return result.status === "allowed" ? result.user : authenticatedApplicationFailureResponse(result);
      },
    };
  }
  ...
  it("creates only opaque encrypted content for an active owner", async () => {
    ...
    const handlers = createPersonalAccountsHandlers({
      ...authenticationDependencies(new FakeSessionVerifier({ subject: "supabase-1", email: "person@example.test" }), { provision: async () => new ApplicationUser("user-1", "supabase", "supabase-1", "person@example.test", "ACTIVE") }),
  ...
  it("does not list accounts for an unauthenticated caller", async () => {
    const handlers = createPersonalAccountsHandlers({
      ...authenticationDependencies(new FakeSessionVerifier(null), { provision: async () => { throw new Error("must not provision"); } }),
