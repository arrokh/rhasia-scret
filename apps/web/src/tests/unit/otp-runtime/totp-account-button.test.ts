...
vi.mock("@/modules/otp-runtime/infrastructure/browser-hmac-generator", () => ({
  BrowserHmacGenerator: class BrowserHmacGenerator {},
}));
...
describe("TotpAccountButton", () => {
  ...
  it("offers an explicit, accessible account-management action", async () => {
    ...
    await act(async () =>
      root?.render(
        createElement(TotpAccountButton, { configuration: configuration(), vaultName: "Brankas Pribadi", onManage }),
      ),
    );
  ...
  it("generates an OTP locally and copies it from the shadcn account card", async () => {
    ...
    await act(async () =>
      root?.render(
        createElement(TotpAccountButton, { configuration: configuration(), vaultName: "Brankas Pribadi", onAccess }),
      ),
    );
  ...
  it("pauses the shared clock while hidden and resynchronizes on visibility", async () => {
    ...
    root = createRoot(container);
    await act(async () =>
      root?.render(createElement(TotpAccountButton, { configuration: configuration(), vaultName: "Brankas Pribadi" })),
    );
  ...
  it("shares one clock and signs only when the TOTP counter changes", async () => {
    ...
    expect(mocks.generateTotp).toHaveBeenCalledTimes(32);
    ...
    expect(mocks.generateTotp).toHaveBeenCalledTimes(64);
  }, 15_000);
});
