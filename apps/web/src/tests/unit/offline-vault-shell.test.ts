...
vi.mock("@/modules/sync/infrastructure/browser-offline-vault-repository", () => ({
  BrowserOfflineVaultRepository: class { clearAll = mocks.clear; listProfiles = mocks.listProfiles; }
}));
...
function workspace() {
  return {
    ...
    vaults: [
      { id: "personal_1", name: "Brankas Pribadi", type: "PERSONAL" as const, role: "OWNER" as const, key: Uint8Array.of(2) },
...
function account(name: string, vaultId: string, vaultName: string, vaultType: "PERSONAL" | "SHARED") {
  return { id: `${name}_account`, vaultId, vaultName, vaultType, revision: 1, issuer: "Example", accountName: `${name}@example.test`, secret: Uint8Array.of(5), algorithm: "SHA-1" as const, digits: 6 as const, period: 30 };
}
