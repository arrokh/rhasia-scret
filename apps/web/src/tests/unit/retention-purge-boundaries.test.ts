...
describe("retention purge architecture", () => {
  ...
  it("keeps purge repositories server-side and never selects encrypted content", () => {
    const accountPurge = readFileSync(join(process.cwd(), "src/modules/authenticator-account/infrastructure/prisma-expired-account-purge-repository.ts"), "utf8");
    const vaultPurge = readFileSync(join(process.cwd(), "src/modules/vault-management/infrastructure/prisma-expired-vault-retention-repository.ts"), "utf8");
