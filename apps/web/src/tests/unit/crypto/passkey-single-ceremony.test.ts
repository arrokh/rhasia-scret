...
class Credential {
  ...
  getClientExtensionResults() { return { prf: { results: { first: this.output.buffer } } }; }
}

describe("passkey unlock ceremony", () => {
  afterEach(() => { vi.unstubAllGlobals(); vi.resetAllMocks(); });

  it.each([false, true])("uses one verified PRF assertion and clears output (verification fails: %s)", async (fails) => {
    ...
    mocks.options.mockResolvedValue({ challenge: "AQID", rpId: "example.test", userVerification: "required", allowCredentials: [{ id: "AQI", type: "public-key" }], encryptedRecoveryPackage: "CQ==" });
    ...
    expect(get).toHaveBeenCalledWith({ publicKey: expect.objectContaining({
      challenge: Uint8Array.of(1, 2, 3), userVerification: "required",
