...
import {
  ...
  RememberedBrowserBindingError,
} from "@/modules/crypto/infrastructure/browser-local-verification";
...
describe("Remembered Browser Local Verification", () => {
  beforeEach(() => {
    ...
    Object.defineProperty(navigator, "credentials", {
      configurable: true,
      value: { create: vi.fn().mockResolvedValue(new FakePublicKeyCredential()) },
    });
  });
  ...
  it("stores only an origin-bound encrypted package after required PRF evaluation", async () => {
    ...
    expect(mocks.saveRemembered).toHaveBeenCalledWith(
      expect.objectContaining({
        version: 1,
        profileId: "profile_1",
        rpId: "localhost",
    ...
    expect(JSON.stringify(mocks.saveRemembered.mock.calls[0]?.[0])).not.toContain(
      btoa(String.fromCharCode(...rootKey)),
    );
  });
  ...
  it("aborts enrollment on lock/navigation and clears temporary PRF output without persisting", async () => {
    ...
    mocks.evaluatePrf.mockReturnValue(
      new Promise<Uint8Array>((resolve) => {
        release = resolve;
      }),
    );
    const enrolling = enrollRememberedBrowser(
      "profile_1",
      Uint8Array.from({ length: 32 }, () => 9),
      controller.signal,
    );
  ...
  it("validates profile origin and RP binding before PRF-gated User Root Key recovery", async () => {
    ...
    const rememberedPackage = {
      version: 1,
      profileId: "profile_1",
      rpId: "localhost",
      origin: "http://localhost:3000",
      credentialId: "AQID",
      encryptedUserRootKeyPackage: btoa(String.fromCharCode(...packageBytes)),
    ...
    await expect(recoverUserRootKeyWithRememberedBrowser("profile_1")).rejects.toBeInstanceOf(
      RememberedBrowserBindingError,
    );
  });
  ...
  it("offers remembered unlock only for a same-site package bound to the requested Personal Vault", async () => {
    ...
    mocks.readRemembered.mockResolvedValue({
      ...
      origin: "http://localhost:3000",
      credentialId: "AQID",
      encryptedUserRootKeyPackage: "AQID",
      enrolledAt: "2026-01-01T00:00:00.000Z",
    });
