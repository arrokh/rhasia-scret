...
describe("passkey recovery package", () => {
  it("requires the WebAuthn PRF output to recover a User Root Key", async () => {
    ...
    await expect(recoverUserRootKeyFromPasskeyPackage(generateSymmetricKey(), packageBytes)).rejects.toThrow(
      "authentication",
    );
    await expect(recoverUserRootKeyFromPasskeyPackage(prfOutput, packageBytes)).resolves.toEqual({
      userRootKey: rootKey,
      prfSalt: salt,
    });
  });
  ...
  it("rejects malformed, oversized, or non-32-byte key material", async () => {
    await expect(
      createPasskeyRecoveryPackage(Uint8Array.of(1), generateSymmetricKey(), generateSymmetricKey()),
    ...
    const malformed = new TextEncoder().encode(
      JSON.stringify({
        version: 1,
        prfSalt: "AQ==",
        encryptedRecoveryWrappingKey: "AQ==",
        encryptedUserRootKey: "AQ==",
        unexpected: true,
      }),
    );
