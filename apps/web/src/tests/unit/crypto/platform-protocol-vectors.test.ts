...
import { ARGON2ID_PROTOCOL_VECTOR, RFC6238_SHA1_VECTOR } from "@rhasia-scret/client-vault-core";
import {
...
describe("cross-platform protocol vectors", () => {
  ...
  it("keeps context-bound encryption, archives, and Secure Share Link material portable", async () => {
    ...
    const context = {
      purpose: "platform-vector",
      payloadType: "synthetic",
      profileId: "profile-vector",
      keyVersion: 1,
    } as const;
    ...
    await expect(decryptPayloadWithContext(key, envelope, { ...context, profileId: "other-profile" })).rejects.toThrow(
    ...
    const wrappedVaultKey = await wrapKeyForRecipientWithContext(key, publicKey, {
    ...
    await expect(
      unwrapKeyForRecipientWithContext(wrappedVaultKey, privateKey, {
        purpose: "vault-key-wrap",
        payloadType: "vault-encryption-key",
        profileId: "profile-vector",
        keyVersion: 1,
