...
function exportedRouteMethods(source: string): string[] {
  return routeMethods.filter((method) => {
    ...
    const aliasedExport = new RegExp(
      `export\\s*\\{[^}]*\\b(?:[A-Za-z_$][\\w$]*\\s+as\\s+)?${method}\\b[^}]*\\}`,
      "s",
    ).test(source);
...
describe("authenticated application mutation rate-limit inventory", () => {
  ...
  it("requires every state-changing route to have a policy or a narrow documented exclusion", () => {
    const discovered = routeFiles(appRoot)
      .flatMap((path) => {
        const source = readFileSync(path, "utf8");
  ...
  it("wires every authenticated mutation route to its declared operation-class policy", () => {
    for (const [route, policy] of Object.entries(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES)) {
      ...
      expect(source, route).toMatch(
        /authenticateApplicationMutation|executeAuthenticatedApplicationRequest|\bauthenticate\(/,
      );
      expect(source, route).toContain(`"${policy}"`);
      expect(source, route).toContain('"fresh-provider-user"');
      expect(source, route).not.toContain("loadApplicationUser");
      expect(source, route).not.toContain("rateLimitApplicationUser");
    }
  });
  ...
  it("shares operation budgets across alternate routes for the same use case", () => {
    ...
    expect(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES["POST /api/shared-vaults/[vaultId]/accounts"]).toBe(
      "account_mutation",
    );
    expect(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES["POST /api/secure-share-links"]).toBe("membership_mutation");
    expect(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES["POST /api/shared-vaults/[vaultId]/share-links"]).toBe(
      "membership_mutation",
    );
    expect(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES["POST /api/passkey-recovery/registration/options"]).toBe(
      "recovery_mutation",
    );
    expect(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES["POST /api/passkey-recovery/registration/verify"]).toBe(
      "recovery_mutation",
    );
  });
  ...
  it("leaves Supabase OTP and magic-link authentication outside application-user limiting", () => {
    ...
    expect(Object.keys(AUTHENTICATED_MUTATION_RATE_LIMIT_POLICIES).some((route) => route.includes("/auth/"))).toBe(
      false,
    );
  });
});
