...
describe("magic-link email delivery boundaries", () => {
  it("keeps Nodemailer and SMTP credentials server-only", () => {
    const clientSources = [
      ...
      "src/modules/identity/presentation/cloudflare-turnstile-widget.tsx",
    ...
    for (const path of clientSources)
      expect(read(path)).not.toMatch(
        /nodemailer|SMTP_PASSWORD|AUTH_MAGIC_LINK_SECRET|AUTH_SESSION_SECRET|TURNSTILE_SECRET_KEY/i,
      );
