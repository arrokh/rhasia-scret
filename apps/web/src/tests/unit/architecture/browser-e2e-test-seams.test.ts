...
} from "@/modules/identity/infrastructure/browser-e2e-passkey-verification";
...
describe("browser E2E test seams", () => {
  ...
  it("accepts deterministic credential responses only behind the development gate", () => {
    ...
    expect(
      browserE2eAuthenticationVerified({ ...authenticationResponse(), id: "wrong" }, Uint8Array.of(1, 2, 3, 4)),
    ).toBe(false);
  });
});
