...
const mocks = vi.hoisted(() => ({
  ...
  recordPersonalAccountCopiesToLocal: vi.fn(),
}));
...
vi.mock("@/modules/audit/server", async (importOriginal) => ({
  ...
  createVaultAuditRepository: () => ({
    ...
    recordAccountAccess: mocks.recordAccountAccess,
    recordArchiveExport: vi.fn(),
    recordPersonalAccountCopiesToLocal: mocks.recordPersonalAccountCopiesToLocal,
  }),
}));
...
describe("Shared Vault audit route", () => {
  ...
  it("records an authorized opaque account-access event and rejects malformed JSON", async () => {
    ...
    const malformed = await POST(new Request("http://localhost/api", { method: "POST", body: "{" }) as never, {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
  ...
  it("records only bounded opaque Personal Vault account targets copied to Local Vault", async () => {
    ...
    const response = await POST_PERSONAL_COPY(
      new NextRequest("http://localhost/api", {
        method: "POST",
        body: JSON.stringify({ eventType: "ACCOUNT_COPIED_TO_LOCAL", accountIds: ["account-1", "account-2"] }),
      }),
      { params: Promise.resolve({ vaultId: "personal-1" }) },
    );
    expect(response.status).toBe(204);
    expect(mocks.recordPersonalAccountCopiesToLocal).toHaveBeenCalledWith("user-1", "personal-1", [
      "account-1",
      "account-2",
    ]);

    const duplicateTargets = await POST_PERSONAL_COPY(
      new NextRequest("http://localhost/api", {
        method: "POST",
        body: JSON.stringify({ eventType: "ACCOUNT_COPIED_TO_LOCAL", accountIds: ["account-1", "account-1"] }),
      }),
      { params: Promise.resolve({ vaultId: "personal-1" }) },
    );
  ...
  it("applies exact account and actor filters before returning redacted owner-only events", async () => {
    mocks.listForOwner.mockResolvedValue({
      ...
      nextCursor: { createdAt: new Date("2026-07-26T12:00:00.000Z"), key: "event-1" },
    });
    const response = await GET(new NextRequest("http://localhost/api?accountId=account-1&actorUserId=viewer-1"), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
    expect(mocks.listForOwner).toHaveBeenCalledWith(
      "user-1",
      "vault-1",
      { accountId: "account-1", actorUserId: "viewer-1" },
    ...
    const nextResponse = await GET(
      new NextRequest(
        `http://localhost/api?accountId=account-1&actorUserId=viewer-1&cursor=${encodeURIComponent(body.nextCursor)}`,
      ),
      { params: Promise.resolve({ vaultId: "vault-1" }) },
    );
    expect(nextResponse.status).toBe(200);
    expect(mocks.listForOwner).toHaveBeenLastCalledWith(
      "user-1",
      "vault-1",
      { accountId: "account-1", actorUserId: "viewer-1" },
      { cursor: { createdAt: new Date("2026-07-26T12:00:00.000Z"), key: "event-1" }, limit: 20 },
    );
    const wrongFilter = await GET(
      new NextRequest(
        `http://localhost/api?accountId=other&actorUserId=viewer-1&cursor=${encodeURIComponent(body.nextCursor)}`,
      ),
      { params: Promise.resolve({ vaultId: "vault-1" }) },
    );
