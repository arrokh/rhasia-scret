...
describe("explicit identity linking", () => {
  it("requires fresh control of both identities and records through the repository", async () => {
    ...
    await linkIdentity(
      { link },
      { applicationUserId: "user-1", existing, proposed, existingReauthenticated: true, proposedReauthenticated: true },
    );
  ...
  it.each([
    ...
    [
      {
        existingReauthenticated: true,
        proposedReauthenticated: true,
        existing: { ...existing, assurance: "fresh-provider-user" as const },
      },
    ...
    [
      {
        existingReauthenticated: true,
        proposedReauthenticated: true,
        proposed: { ...proposed, assurance: "fresh-provider-user" as const },
      },
      "active provider verification",
    ],
    ...
    const link = vi.fn();
    await expect(
      linkIdentity({ link }, { applicationUserId: "user-1", existing, proposed, ...override }),
