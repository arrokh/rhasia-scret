...
import { createAuthenticatedApplicationExecutor, type AuthenticatedApplicationDependencies } from "@/modules/server-composition";
...
function dependencies(overrides: Partial<AuthenticatedApplicationDependencies> = {}) {
  const value: AuthenticatedApplicationDependencies = {
    ...
    provisionApplicationUser: vi.fn(async () => new ApplicationUser("user_1", principal.issuer, principal.subject, principal.email, "ACTIVE")),
...
describe("authenticated application request", () => {
  ...
  it("distinguishes a missing session from a missing Application User", async () => {
    ...
    const noUser = createAuthenticatedApplicationExecutor(dependencies({ provisionApplicationUser: vi.fn(async () => null) }));
    await expect(noSession({ assurance: "fresh-provider-user", access: "reader" })).resolves.toEqual({ status: "unauthenticated" });
    await expect(noUser({ assurance: "fresh-provider-user", access: "reader" })).resolves.toEqual({ status: "application_user_unavailable" });
  });

  it("rejects inactive users before consuming a mutation budget", async () => {
    const deps = dependencies({
      provisionApplicationUser: vi.fn(async () => new ApplicationUser("user_1", principal.issuer, principal.subject, principal.email, "INACTIVE"))
    });
    const execute = createAuthenticatedApplicationExecutor(deps);
    await expect(execute({ assurance: "fresh-provider-user", access: "mutation", operation: "account_mutation" })).resolves.toEqual({ status: "inactive_user" });
    expect(deps.checkApplicationRateLimit).not.toHaveBeenCalled();
  });

  it("keeps readers outside mutation budgets", async () => {
    const deps = dependencies();
    const result = await createAuthenticatedApplicationExecutor(deps)({ assurance: "verified-claims", access: "reader" });
  ...
  it.each([
    ...
    [{ status: "unavailable", retryAfterSeconds: 5 } as const, { status: "rate_limit_unavailable", retryAfterSeconds: 5 }]
    ...
    const execute = createAuthenticatedApplicationExecutor(deps);
    await expect(execute({ assurance: "fresh-provider-user", access: "mutation", operation: "membership_mutation" })).resolves.toEqual(expected);
