...
import {
  ...
  VaultArchiveExportWorkspace,
} from "@/modules/vault-archive/presentation/vault-archive-exporter";

Object.assign(globalThis, { IS_REACT_ACT_ENVIRONMENT: true });
const mocks = vi.hoisted(() => ({ recordVaultArchiveExport: vi.fn() }));
vi.mock("@/modules/audit", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/modules/audit")>()),
  recordVaultArchiveExport: mocks.recordVaultArchiveExport,
}));
...
const viewerPermissions = {
  ...
  sources: { canAddAccounts: "VAULT" as const, canEditAccounts: "VAULT" as const, canDeleteAccounts: "VAULT" as const },
};
const workspace = {
  profileId: "profile-1",
...
describe("VaultArchiveExporter", () => {
  ...
  it("excludes Viewer Vaults and releases archive/key only after audit succeeds", async () => {
    ...
    mocks.recordVaultArchiveExport.mockImplementation(
      () =>
    ...
    await act(async () =>
      root?.render(
        createElement(
          ...
          { initialWorkspace: { ...workspace, syncState: "STALE" } },
          createElement(VaultArchiveExportWorkspace, { personalVaultId: "personal-1" }),
        ),
      ),
    );
  ...
  it("does not release archive/key when audit recording fails", async () => {
    ...
    await act(async () => {
      ...
      await vi.waitFor(() =>
        expect(container.textContent).toContain("Cadangan terenkripsi tidak dapat dibuat atau dicatat dalam audit."),
      );
    });
    expect(container.textContent).not.toContain("Unduh arsip");
