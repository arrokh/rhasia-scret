...
const mocks = vi.hoisted(() => ({
  user: { id: "recipient-1", email: "recipient@example.test", canAccessApplication: () => true } as {
    id: string;
    email: string;
...
vi.mock("@/modules/identity/application/load-application-user", () => ({
  loadApplicationUser: async () => mocks.user,
}));
...
vi.mock("@/modules/vault-membership/infrastructure/prisma-secure-share-link-repository", () => ({
  PrismaSecureShareLinkRepository: class {
    findForRecipient = mocks.findForRecipient;
    redeem = mocks.redeem;
  },
}));
...
describe("Secure Share Link route", () => {
  ...
  it("looks up and redeems a link using the authenticated user id and verified email", async () => {
    mocks.findForRecipient.mockResolvedValue({
      id: "invitation-1",
      vaultId: "vault-1",
      encryptedPackage: new Uint8Array(13).fill(2),
    });
    const getResponse = await GET(
      new NextRequest(`http://localhost/api/secure-share-links?verifier=${encodeURIComponent(verifier)}`),
    );
    expect(getResponse.status).toBe(200);
    expect(mocks.findForRecipient).toHaveBeenCalledWith(
      { userId: "recipient-1", email: "recipient@example.test" },
  ...
  it("rejects inactive users and malformed redemption JSON", async () => {
    ...
    await expect(
      GET(new NextRequest(`http://localhost/api/secure-share-links?verifier=${encodeURIComponent(verifier)}`)),
    ).resolves.toMatchObject({ status: 403 });
    expect(mocks.findForRecipient).not.toHaveBeenCalled();

    mocks.user = { id: "recipient-1", email: "recipient@example.test", canAccessApplication: () => true };
    await expect(
      POST(new NextRequest("http://localhost/api/secure-share-links", { method: "POST", body: "{" })),
...
function validRedemptionRequest() {
  return new NextRequest("http://localhost/api/secure-share-links", {
    ...
    body: JSON.stringify({
      ...
      encryptedVaultKey: Buffer.alloc(13, 2).toString("base64"),
      keyVersion: 1,
    }),
  });
}
