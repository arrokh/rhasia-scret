...
const mocks = vi.hoisted(() => ({
  readVaultDefaults: vi.fn(),
  updateVaultDefaults: vi.fn(),
  updateMemberOverrides: vi.fn(),
}));
vi.mock("@/modules/identity/application/load-application-user", () => ({
  loadApplicationUser: async () => ({ id: "owner-1", canAccessApplication: () => true }),
}));
...
import {
  GET as loadDefaults,
  PATCH as updateDefaults,
} from "@/app/api/shared-vaults/[vaultId]/member-permissions/route";
...
describe("Shared Vault permission routes", () => {
  ...
  it("loads only the Vault-wide defaults needed by the Detail tab", async () => {
    ...
    const response = await loadDefaults(new Request("http://localhost/api"), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      vaultDefaultAccountPermissions: permissions,
  ...
  it("updates a complete Vault-wide default profile with revision protection", async () => {
    ...
    const response = await updateDefaults(request({ expectedRevision: 2, ...permissions }), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      vaultDefaultAccountPermissions: permissions,
      vaultDefaultAccountPermissionsRevision: 3,
    });
  ...
  it("updates nullable member overrides independently", async () => {
    ...
    const response = await updateMember(request({ expectedRevision: 3, ...overrides }), {
      params: Promise.resolve({ vaultId: "vault-1", userId: "member-1" }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      permissionOverrides: overrides,
      effectiveAccountPermissions: effective,
      permissionsRevision: 4,
    });
  ...
  it("rejects partial or extra permission payloads", async () => {
    const partial = await updateMember(request({ expectedRevision: 1, canAddAccounts: null, canEditAccounts: true }), {
      params: Promise.resolve({ vaultId: "vault-1", userId: "member-1" }),
    });
    expect(partial.status).toBe(400);
    const extra = await updateDefaults(request({ expectedRevision: 1, ...permissions, role: "OWNER" }), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
  ...
  it("maps stale and unavailable updates without exposing Vault details", async () => {
    mocks.updateVaultDefaults
      .mockResolvedValueOnce({ status: "STALE" })
      .mockResolvedValueOnce({ status: "UNAVAILABLE" });
    const stale = await updateDefaults(request({ expectedRevision: 1, ...permissions }), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
    expect(stale.status).toBe(409);
    const unavailable = await updateDefaults(request({ expectedRevision: 1, ...permissions }), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
