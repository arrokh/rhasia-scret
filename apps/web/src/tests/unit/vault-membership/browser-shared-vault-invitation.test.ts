...
vi.mock("@/modules/vault-membership/infrastructure/browser-secure-share-link", () => ({
  createSecureShareLinkMaterial: mocks.createSecureShareLinkMaterial,
}));
...
describe("createSharedVaultInvitation", () => {
  ...
  it("sends only recipient identity and encrypted one-time material to the server", async () => {
    ...
    mocks.createSecureShareLinkMaterial.mockResolvedValue({
