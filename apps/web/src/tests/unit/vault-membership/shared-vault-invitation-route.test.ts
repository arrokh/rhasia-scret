...
describe("POST Shared Vault invitation", () => {
  ...
  it("binds encrypted one-time link material to an invited email", async () => {
    ...
    expect(mocks.createForEmail).toHaveBeenCalledWith(
      ...
      "vault-1",
      "viewer@example.test",
      expect.objectContaining({ linkVerifier: expect.any(Uint8Array), encryptedPackage: expect.any(Uint8Array) }),
    );
  });
  ...
  it("maps malformed JSON and invitation conflicts without hiding infrastructure failures", async () => {
    ...
    await expect(POST(validRequest(), { params: Promise.resolve({ vaultId: "vault-1" }) })).resolves.toMatchObject({
      status: 409,
    });

    mocks.createForEmail.mockRejectedValueOnce(new Error("database unavailable"));
    await expect(POST(validRequest(), { params: Promise.resolve({ vaultId: "vault-1" }) })).rejects.toThrow(
