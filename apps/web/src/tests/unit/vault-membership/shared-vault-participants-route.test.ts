...
vi.mock("@/modules/vault-membership/infrastructure/prisma-vault-participant-repository", () => ({
  PrismaVaultParticipantRepository: class {
...
describe("Shared Vault participant routes", () => {
  ...
  it("returns owner-visible members and pending invitations", async () => {
    mocks.listForOwner.mockResolvedValue({
      ...
      items: [
        {
          ...
          invitedAt: new Date("2026-07-26T12:00:00.000Z"),
          expiresAt: null,
      ...
      nextCursor: { createdAt: new Date("2026-07-26T12:00:00.000Z"), key: "member:user-1" },
    });
    ...
    await GET(new Request(`http://localhost/api?cursor=${encodeURIComponent(body.nextCursor)}&limit=1`), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
    expect(mocks.listForOwner).toHaveBeenLastCalledWith("owner-1", "vault-1", {
      cursor: { createdAt: new Date("2026-07-26T12:00:00.000Z"), key: "member:user-1" },
      limit: 1,
    });
  });
  ...
  it("rejects a structurally invalid participant cursor", async () => {
    const cursor = encodeTimestampCursor(
      { createdAt: new Date("2026-07-26T12:00:00.000Z"), key: "member:" },
      "vault-participants:vault-1",
    );
    const response = await GET(new Request(`http://localhost/api?cursor=${encodeURIComponent(cursor)}`), {
      params: Promise.resolve({ vaultId: "vault-1" }),
    });
  ...
  it("deletes only an exact pending invitation owned by the caller", async () => {
    ...
    await expect(
      DELETE(new Request("http://localhost/api", { method: "DELETE" }), {
        params: Promise.resolve({ vaultId: "vault-1", invitationId: "other" }),
      }),
