...
vi.mock("@/modules/vault-membership/infrastructure/prisma-membership-lifecycle-repository", () => ({
  PrismaMembershipLifecycleRepository: class {
    revoke = mocks.revoke;
  },
}));
  ...
  it("revokes the exact member and only maps domain unavailability to 404", async () => {
    ...
    await expect(
      DELETE(new Request("http://localhost/api", { method: "DELETE" }), {
        params: Promise.resolve({ vaultId: "vault-1", userId: "viewer-1" }),
      }),
    ...
    await expect(
      DELETE(new Request("http://localhost/api", { method: "DELETE" }), {
        params: Promise.resolve({ vaultId: "vault-1", userId: "viewer-2" }),
      }),
    ...
    await expect(
      DELETE(new Request("http://localhost/api", { method: "DELETE" }), {
        params: Promise.resolve({ vaultId: "vault-1", userId: "viewer-3" }),
      }),
