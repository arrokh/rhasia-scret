...
vi.mock("@/shared/infrastructure/browser-api-client", () => ({ browserAuthenticatedTransport: { request: mocks.request } }));
...
describe("redeemSecureShareLink", () => {
  ...
  it("uses a verifier for lookup and sends only the recipient-wrapped Vault Encryption Key", async () => {
    ...
    const lookup = { id: "invitation-1", vaultId: "vault-1", encryptedPackage: Buffer.from(encryptedPackage).toString("base64") };
    mocks.request.mockImplementation(async (request: { method: string }) => response(request.method === "GET" ? 200 : 204, request.method === "GET" ? lookup : null));
    ...
    expect(mocks.request).toHaveBeenNthCalledWith(1, expect.objectContaining({ url: expect.stringMatching(/^\/api\/secure-share-links\?verifier=/), method: "GET", cache: "no-store" }));
    ...
    expect(mocks.request).toHaveBeenNthCalledWith(2, expect.objectContaining({
      ...
      method: "POST",
      body: JSON.stringify({ invitationId: "invitation-1", encryptedVaultKey: expectedEncryptedVaultKey, keyVersion: 1 })
    }));
  });
