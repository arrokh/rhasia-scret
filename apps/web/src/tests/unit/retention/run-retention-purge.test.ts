...
describe("runRetentionPurge", () => {
  it("drains bounded batches in policy order and reports only opaque identifiers", async () => {
    ...
    const vaults = { purgeExpiredVaults: vi.fn(async () => { order.push("vaults"); return { purgedIds: vaultBatches.shift() ?? [] }; }) };
    const audit = { purgeExpiredAuditEvents: vi.fn(async () => { order.push("audit"); return { purgedIds: auditBatches.shift() ?? [] }; }) };

    const report = await runRetentionPurge({ accounts, vaults, audit, now: new Date("2026-01-01T00:00:00.000Z"), batchSize: 2, maxBatches: 3 });
  ...
  it("stops at the configured maximum and flags possible backlog without unbounded work", async () => {
    ...
    expect(audit.purgeExpiredAuditEvents).toHaveBeenCalledTimes(2);
