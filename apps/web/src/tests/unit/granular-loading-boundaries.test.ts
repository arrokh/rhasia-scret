...
describe("granular loading boundaries", () => {
  ...
  it("scopes action and body fallbacks to independent Suspense boundaries", () => {
    const source = readFileSync("src/app/vaults/vault-page-frame.tsx", "utf8");
  ...
  it("uses exact-region placeholders for permission defaults, participants, and audit history", () => {
    const manager = readFileSync("src/modules/vault-management/presentation/shared-vault-manager.tsx", "utf8");
    const audit = readFileSync("src/modules/vault-management/presentation/vault-audit-history.tsx", "utf8");
    expect(manager).toContain("defaults.isPending");
    expect(manager).toContain("<FormLoadingPlaceholder");
    expect(manager).toContain("<SectionLoadingPlaceholder rows={2}");
