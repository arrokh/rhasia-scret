...
describe("AuthenticatorAccountManagerDialog", () => {
  ...
  it("encrypts and revision-updates an edited account label", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ id: "account-1", revision: 2 }), {
        status: 200,
  ...
  it("renders edit and delete capabilities independently", async () => {
    ...
    expect([...document.body.querySelectorAll("button")].some((button) => button.textContent === "Simpan label")).toBe(
      false,
    );
  ...
  it("requires a second modal confirmation before deleting", async () => {
    ...
    const confirmation =
      document.body.querySelectorAll('[role="dialog"]')[1] ?? document.body.querySelector('[role="alertdialog"]');
  ...
  await act(async () =>
    root.render(
      createElement(
        TestQueryProvider,
        null,
        createElement(AuthenticatorAccountManagerDialog, {
          account: {
            id: "account-1",
            vaultId: "personal-1",
