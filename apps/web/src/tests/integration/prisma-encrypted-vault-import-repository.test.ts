...
describe("PrismaEncryptedVaultImportRepository", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      await expect(new PrismaEncryptedVaultImportRepository().import(randomUUID(), request)).resolves.toEqual({
        status: "CONFLICT",
      });
    },
  );
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      await expect(new PrismaEncryptedVaultImportRepository().import(owner.id, request)).resolves.toEqual({
        status: "IMPORTED",
        vaultId: vault.id,
