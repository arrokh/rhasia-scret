...
describe("PrismaPersonalAccountRepository recovery", () => {
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      const account = await new PrismaPersonalAccountRepository().create(user.id, vault.id, {
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      await expect(
        new PrismaPersonalAccountRepository(() => deletedAt).delete(user.id, vault.id, account.id, 1),
      ).resolves.toBe(true);
      await expect(
        new PrismaPersonalAccountRepository(() => new Date("2026-08-25T23:59:59.999Z")).restore(
          user.id,
          vault.id,
          account.id,
        ),
      ...
      await expect(
        new PrismaPersonalAccountRepository(() => deletedAt).delete(user.id, vault.id, account.id, 3),
      ).resolves.toBe(true);
      await expect(
        new PrismaPersonalAccountRepository(() => new Date("2026-08-26T00:00:00.000Z")).restore(
          user.id,
          vault.id,
          account.id,
        ),
