...
describe("Prisma retention purge", () => {
  ...
  it.skipIf(!process.env.DATABASE_URL)("sets and clears account deadlines through delete and restore", async () => {
    ...
    await expect(
      new PrismaSharedAccountRepository(() => new Date("2026-08-25T11:59:59.999Z")).restore(
        owner.id,
        vault.id,
        account.id,
      ),
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      const deletedAt = new Date("2026-07-26T12:00:00.000Z");
      await expect(new PrismaSharedVaultRecoveryRepository(() => deletedAt).delete(owner.id, vault.id)).resolves.toBe(
        true,
      );
      ...
      await expect(
        new PrismaVaultAuditRepository(() => new Date("2027-07-26T12:00:00.000Z")).listForOwner(owner.id, vault.id),
      ).resolves.toBeNull();
    },
  );
  ...
  it.skipIf(!process.env.DATABASE_URL)(
    ...
    async () => {
      ...
      const [restoredAccount, purgedAccounts] = await Promise.all([
        new PrismaSharedAccountRepository(() => new Date(deadline.getTime() - 1)).restore(
          owner.id,
          vault.id,
          account.id,
        ),
