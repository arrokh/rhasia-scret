...
describe("Prisma application rate-limit repository", () => {
  ...
  it.skipIf(!process.env.DATABASE_URL)("retains only a bounded period of expired aggregate windows", async () => {
    ...
    const now = Date.now();
    await prisma.applicationRateLimitWindow.createMany({ data: [
      { userId: user.id, operation: "old_metric", windowStartedAt: new Date(now - 49 * 60 * 60 * 1_000), expiresAt: new Date(now - 48 * 60 * 60 * 1_000), requestCount: 1 },
      { userId: user.id, operation: "recent_metric", windowStartedAt: new Date(now - 2 * 60 * 60 * 1_000), expiresAt: new Date(now - 60 * 60 * 1_000), requestCount: 1 }
    ] });

    await new PrismaApplicationRateLimitRepository().consume(user.id, "audit_event", { limit: 1, windowSeconds: 60 });

    await expect(prisma.applicationRateLimitWindow.findMany({ where: { userId: user.id }, select: { operation: true } })).resolves.toEqual(expect.arrayContaining([
      { operation: "recent_metric" },
      { operation: "audit_event" }
    ]));
    expect(await prisma.applicationRateLimitWindow.findFirst({ where: { userId: user.id, operation: "old_metric" } })).toBeNull();
  });

  it.skipIf(!process.env.DATABASE_URL)("isolates users and operation classes while sharing alternate-route budgets", async () => {
...
async function createUser(label: string) {
  ...
  const user = await prisma.applicationUser.create({ data: { supabaseUserId: `${label}-${suffix}`, email: `${label}-${suffix}@example.test` } });
