...
export default async function MobilePreviewPage() {
  ...
  return <AppPage>
    ...
    <SurfaceCard className="grid min-w-0 gap-5 p-4 sm:p-5" aria-label={t("listLabel")}>
      <div className="flex min-w-0 items-center gap-2" data-slot="vault-account-actions"><Button variant="outline" asChild><Link href="/vaults/manage" aria-label={t("vaults")} title={t("vaults")}><Vault aria-hidden="true" /><span className="hidden md:inline">{t("vaults")}</span></Link></Button><Button variant="outline" aria-label={t("localAction")} title={t("localAction")}><ArrowLeftRight aria-hidden="true" /><span className="hidden md:inline">{t("localAction")}</span></Button><Button variant="outline" aria-label={t("securityAction")} title={t("securityAction")}><ShieldKeyhole aria-hidden="true" /><span className="hidden md:inline">{t("securityAction")}</span></Button><Button asChild className="ml-auto"><Link href="/vaults/accounts/new" aria-label={t("addLabel")} title={t("add")}><Plus /><span className="hidden md:inline">{t("add")}</span></Link></Button></div>
