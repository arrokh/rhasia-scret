...
describe("mobile Secure Share Links", () => {
  ...
  it("keeps link material client-only and sends only verifier and recipient-wrapped Vault key", async () => {
    ...
    const encryptedPackage = nativeClientCrypto.serializeEncryptedEnvelope(await nativeClientCrypto.encryptPayloadWithContext(linkKey, vaultKey, {
    ...
    linkKey.fill(0);
    const transport = new ShareLinkTransport({ id: "invitation_1", vaultId, encryptedPackage: bytesToBase64(encryptedPackage) });
