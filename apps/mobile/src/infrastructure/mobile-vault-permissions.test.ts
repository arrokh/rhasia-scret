...
describe("mobile Shared Vault permission defaults", () => {
  it("preserves optimistic revisions through authorized transport", async () => {
    ...
    const updated = await updateMobileVaultPermissionDefaults(
      "shared_1",
      { ...loaded, permissions: { ...loaded.permissions, canDeleteAccounts: true } },
      transport,
    );
...
class StubTransport implements AuthenticatedTransport {
  ...
  public async request(request: PlatformHttpRequest): Promise<PlatformHttpResponse> {
    ...
    const body = {
      vaultDefaultAccountPermissions: { canAddAccounts: true, canEditAccounts: false, canDeleteAccounts: patch },
      vaultDefaultAccountPermissionsRevision: patch ? 3 : 2,
    };
    return {
      status: 200,
      ok: true,
