...
export async function loadMobileVaultPermissionDefaults(
  vaultId: string,
...
export async function updateMobileVaultPermissionDefaults(
  vaultId: string,
  current: MobileVaultPermissionDefaults,
  transport: AuthenticatedTransport,
): Promise<MobileVaultPermissionDefaults> {
  const response = await transport.request({
    url: `/api/shared-vaults/${encodeURIComponent(vaultId)}/member-permissions`,
...
function parse(value: unknown): MobileVaultPermissionDefaults {
  ...
  if (
    typeof values.canAddAccounts !== "boolean" ||
    typeof values.canEditAccounts !== "boolean" ||
    typeof values.canDeleteAccounts !== "boolean" ||
    typeof record.vaultDefaultAccountPermissionsRevision !== "number" ||
    !Number.isSafeInteger(record.vaultDefaultAccountPermissionsRevision) ||
    record.vaultDefaultAccountPermissionsRevision < 1
  )
    invalid();
  return {
    permissions: {
      canAddAccounts: values.canAddAccounts,
      canEditAccounts: values.canEditAccounts,
      canDeleteAccounts: values.canDeleteAccounts,
    },
    revision: record.vaultDefaultAccountPermissionsRevision,
  };
}

function invalid(): never {
  throw new Error("Shared Vault permission response is invalid.");
}
