...
describe("NativeCryptoValidation", () => {
  ...
  it("renders bilingual passing evidence for the shared synthetic vector", async () => {
    const expected = Uint8Array.from(ARGON2ID_PROTOCOL_VECTOR.expectedHex.match(/.{2}/g) ?? [], (value) =>
    ...
    expect(
      screen.getByRole("header", { name: "Validasi kriptografi native / Native cryptography validation" }),
    ).toBeVisible();
