...
import {
  loadMobileVaultPermissionDefaults,
  updateMobileVaultPermissionDefaults,
  type MobileVaultPermissionDefaults,
} from "../infrastructure/mobile-vault-permissions";
import type { MobileMessages } from "../localization";

export function MobileSharedVaults({
  copy,
  webOrigin,
  workspace,
  transport,
  ...
  const workspaceWritable = canMutateVault(
    resolveVaultStatus({
      origin: "PERSONAL",
          ...
          {vault.role === "OWNER" && workspaceWritable ? (
            <View style={styles.actions}>
              <Pressable
                ...
                onPress={() =>
                  void loadMobileVaultAuditEvents(vault.id, transport).then(
                    (events) => {
              ...
              <Pressable
                accessibilityRole="button"
                onPress={() =>
                  void loadMobileVaultPermissionDefaults(vault.id, transport).then(
                    (defaults) => {
                      setPermissionVaultId(vault.id);
        ...
        <View style={styles.section}>
          ...
          {(["canAddAccounts", "canEditAccounts", "canDeleteAccounts"] as const).map((permission) => (
            <Pressable
              ...
              accessibilityState={{ checked: permissionDefaults.permissions[permission] }}
              key={permission}
              onPress={() =>
                setPermissionDefaults({
                  ...
                  permissions: {
                    ...
                    [permission]: !permissionDefaults.permissions[permission],
                  },
                })
              }
              style={styles.button}
            >
              <Text style={styles.buttonText}>
                {permissionLabel(permission, copy)}:{" "}
                {permissionDefaults.permissions[permission] ? copy.allowed : copy.notAllowed}
              </Text>
            </Pressable>
          ))}
          <Pressable
            accessibilityRole="button"
            onPress={() =>
              void updateMobileVaultPermissionDefaults(permissionVaultId, permissionDefaults, transport).then(
                (updated) => {
                  setPermissionDefaults(updated);
                  setPermissionError(false);
                },
                () => setPermissionError(true),
              )
            ...
            style={styles.button}
          >
            <Text style={styles.buttonText}>{copy.savePermissions}</Text>
          </Pressable>
        </View>
      ) : null}
      {permissionError ? (
        <Text accessibilityLiveRegion="assertive" style={styles.error}>
          {copy.memberPermissionsError}
        </Text>
      ) : null}
      {auditEvents.length ? (
        <View style={styles.section}>
          {auditEvents.map((event) => (
            <View key={event.id} style={styles.card}>
              <Text style={styles.name}>{auditLabel(event.eventType, copy)}</Text>
              <Text style={styles.guidance}>
                {event.actorEmail} · {new Date(event.createdAt).toLocaleString(copy.dateLocale)}
              </Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.actions}>
            {owned.map((vault) => (
              <Pressable
                accessibilityRole="button"
                accessibilityState={{ selected: vault.id === vaultId }}
                key={vault.id}
                onPress={() => setVaultId(vault.id)}
                style={styles.button}
              >
          ...
          <form.Field
            ...
            validators={{
              onSubmit: ({ value }) =>
                /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.trim()) ? undefined : copy.recipientEmailInvalid,
            }}
          >
            {(field) => {
              ...
              return (
                <View style={styles.section}>
                  <TextInput
                    accessibilityLabel={copy.recipientEmail}
          ...
          >
            <Text style={styles.buttonText}>
              {status === "busy" ? copy.creatingSecureShareLink : copy.createAndShareLink}
            </Text>
          </Pressable>
          {status === "success" ? (
            <Text accessibilityLiveRegion="polite" style={styles.guidance}>
              {copy.secureShareLinkCreated}
            </Text>
          ) : null}
          {status === "error" ? (
            <Text accessibilityLiveRegion="assertive" style={styles.error}>
              {copy.secureShareLinkCreateError}
            </Text>
          ) : null}
        </View>
      ) : null}
...
function permissionLabel(
  permission: "canAddAccounts" | "canEditAccounts" | "canDeleteAccounts",
  copy: MobileMessages,
): string {
...
function permissionSummary(
  permissions: { canAddAccounts: boolean; canEditAccounts: boolean; canDeleteAccounts: boolean },
  copy: MobileMessages,
): string {
  return (
    [
      permissions.canAddAccounts ? copy.permissionAdd : null,
      permissions.canEditAccounts ? copy.permissionEdit : null,
      permissions.canDeleteAccounts ? copy.permissionDelete : null,
    ]
      .filter(Boolean)
      .join(" · ") || copy.permissionReadOnly
  );
}
