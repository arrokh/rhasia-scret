...
- Mobile tests, fixtures, and examples use synthetic, non-PII values only; never copy user-provided account labels, issuer names, emails, `otpauth` URIs, QR payloads, or credentials into the repository.
...
- Authorized encrypted Personal Vault snapshots receive a second context-authenticated encryption layer in `expo-file-system`; its random key remains in Keychain/Android Keystore through `expo-secure-store`. Shared Vault ciphertext and keys are never persisted in the offline store. Offline snapshots expose local TOTP generation and explicit `expo-clipboard` copy but no mutations, queue, replay, merge, upload, or deletion propagation; legacy Shared-containing stores are removed before offline presentation.
