# Roadmap

This roadmap records public-launch work without promising dates. Security,
legal, deployment, privacy, and release evidence can hold a milestone even
when implementation code is complete.

## Public-launch foundation

- [x] Public README and documentation index — [#143](https://github.com/arrokh/rhasia-scret/issues/143)
- [x] Pull-request CI and security automation — [#145](https://github.com/arrokh/rhasia-scret/issues/145)
- [x] Supported self-hosting guide — [#146](https://github.com/arrokh/rhasia-scret/issues/146)
- [ ] Project license and third-party provenance — [#142](https://github.com/arrokh/rhasia-scret/issues/142)
- [ ] Privacy and hosted-service disclosures — [#147](https://github.com/arrokh/rhasia-scret/issues/147)
- [ ] Contributor workflow and governance — [#148](https://github.com/arrokh/rhasia-scret/issues/148)
- [ ] Versioning and web/mobile release process — [#149](https://github.com/arrokh/rhasia-scret/issues/149)
- [ ] Public-launch audit and first release — [#150](https://github.com/arrokh/rhasia-scret/issues/150)

## Priorities

Security and dependency blockers, documented provider responsibility, and
reproducible release evidence take priority over feature expansion. The
[MVP plan](docs/mvp-plan.md) and architecture/security ADRs remain the source
of truth for product scope and technical boundaries.

## How to propose a change

Open a focused issue or pull request using the public templates. Explain the
user or operator outcome, affected bounded context, security and localization
impact, and verification evidence. Roadmap status changes require a linked
issue or merged change; release status requires the governance approval path.
