...
**PWA Authentication Handoff**:
The short-lived same-origin bridge for an installed web PWA whose cookie jar may be separate from the browser that opens an email link. The browser redeems a PWA magic link, sends the resulting refresh credential only in the same-origin PWA-session publisher request, and the PWA polls the verifier-backed handoff endpoint to receive HttpOnly session cookies. If the callback opens in a new PWA window, the original sign-in window supplies the session-scoped verifier through a one-shot, handoff-ID-matched BroadcastChannel exchange; the refresh credential never crosses that channel. The credential is never written to browser storage, service-worker caches, analytics, logs, or server persistence; only keyed handoff digests, the normalized request email, and lifecycle metadata are persisted, and a failed or unavailable handoff fails closed.
_Avoid_: shared cookie assumption, access-token URL handoff, persisted PWA session credential
