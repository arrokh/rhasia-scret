...
- [ ] Dynamic HTML/API responses contain a nonce-based CSP; static assets contain the restrictive fallback policy. Confirm `script-src` has no production `unsafe-inline` or `unsafe-eval`; `script-src-elem` permits only same-origin immutable Next.js chunks, the configured PostHog asset host, and the Cloudflare Web Analytics beacon host while inline scripts still require the response nonce. `connect-src` permits the corresponding PostHog and Cloudflare Web Analytics reporting endpoints. The only WebAssembly exception is the narrow `wasm-unsafe-eval` needed by the Argon2id browser implementation.
...
- [ ] Supabase PKCE/cookie configuration, public signup and Confirm email settings, access/refresh lifetime and reuse, verified-email enforcement, session revocation, logout scope, and user switching are verified against the current provider documentation.
