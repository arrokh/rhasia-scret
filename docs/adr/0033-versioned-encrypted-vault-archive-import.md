...
The user may select their Personal Vault, an active Shared Vault for which their Effective Shared Vault Account Permissions allow account creation, or create a new Shared Vault named from the archive preview. ADR-0047 therefore amends the original owner-only existing-Shared-Vault destination rule; creating a new Shared Vault remains owner-only by construction. Duplicate detection uses plaintext only in client memory and requires an explicit cancel or add-anyway choice.
