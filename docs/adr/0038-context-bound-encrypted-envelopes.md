...
| Local Vault Snapshot                                | the ordinary contained Personal Vault profile/name/key/account contexts; Shared Vault contexts are never contained                                   | Snapshot remains an encrypted, read-only Personal-Vault-only server-derived copy. Native storage adds a separate outer `native-offline-snapshot` context-authenticated envelope; its random storage key remains in secure native storage and is not the Vault key. |
