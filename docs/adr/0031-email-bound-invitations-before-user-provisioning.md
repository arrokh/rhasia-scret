...
Owners can create Invitations for an exact email recipient before that person signs up or completes cryptographic enrollment. The database learns the invited email as permitted authorization metadata, but it never receives the Secure Share Link secret, usable Vault Encryption Key, plaintext Vault name, Authenticator Account content, or OTP. A person may create a hosted Application User through passwordless sign-in, but cannot redeem an Invitation unless the authenticated session's verified email matches that Invitation.
