...
An unlocked user may detach Passkey-Assisted Recovery. Detachment deletes the server-held credential metadata, encrypted recovery package, and outstanding recovery challenges, so the credential can no longer recover or unlock the Vault. It does not change the Vault Unlock Secret or encrypted Vault content. WebAuthn does not provide a relying party API for deleting a credential from an authenticator, so a detached passkey entry may remain in the operating system or password manager until the user removes it there.
