...
Explicit application events use a typed allowlist and locale-independent, aggregate properties. They cannot accept arbitrary event names or Vault content; runtime sanitization independently validates event names, enum values, and bounded counts. Web Vitals are explicitly enabled for LCP, CLS, FCP, and INP with attribution and network timing disabled. Automatic exception tracking captures unhandled errors and promise rejections but not console logs; before_send retains only a bounded exception type, never messages, URLs, or stacks. PostHog Logs console capture is explicitly disabled because browser log records include current URL metadata outside the event sanitizer. The application sends no browser log body until a safe, URL-redacting PostHog Logs integration is available. The authenticated-session event is emitted only after the browser identifies the hashed application user; the pre-authentication sign-in-link event remains anonymous and is not treated as part of an identified activation funnel. Error-boundary telemetry is limited to bounded error names and digests. The current product event catalog and recommended PostHog views are maintained in `docs/analytics-events.md`.
