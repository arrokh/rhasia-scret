...
Grouping alternate routes into one operation class prevents endpoint-path changes from multiplying a user's effective budget. Rate limiting runs only after passwordless/OIDC session verification and Application User loading, and before request-body parsing or domain mutation. It supplements authorization, optimistic revision checks, one-time Secure Share Link semantics, and the passwordless service's own link-request limits.
