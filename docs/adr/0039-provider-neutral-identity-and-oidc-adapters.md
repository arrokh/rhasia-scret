...
- `passwordless`: self-managed email-link authentication. One-time link challenges, local sessions, native/PWA refresh rotation, browser assertion keepalive, revocation, and anonymous abuse limits are persisted through the application database; email is delivered by the server-only SMTP adapter.
