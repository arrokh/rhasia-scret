...
| Stolen web or native session | Passwordless/OIDC session verification, configured Application Admission (verified email when configured), application-user status checks, bearer/cookie transport boundaries, and re-unlock before access to decrypted material. |
