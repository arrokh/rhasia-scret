...
| `EMAIL_PROVIDER_URL`, `EMAIL_PROVIDER_TOKEN`, `AUTH_EMAIL_FROM`, `AUTH_EMAIL_FROM_NAME`           | API passwordless runtime                 | Server-only HTTP email provider settings.                                                                                                                                                         |
...
Run `pnpm run verify:deployment-config` before deployment. Use `VERIFY_DEPLOYMENT_PRODUCTION=1` to enforce production requirements; use `DEPLOYMENT_TARGET=worker` for the Cloudflare API and `DEPLOYMENT_TARGET=bun` (the default) for self-hosting. It validates URL shape, backend configuration, API proxy credentials, secret length, and conditional variables without printing their values. Worker validation rejects direct database URLs; Bun validation requires the runtime and migration endpoints.
...
Set `AUTH_BACKEND=passwordless`, the API-only `AUTH_*` values, and the server-only HTTP email-provider values. The user flow is:
