...
| API host              | Cloudflare Worker                                 | Bun self-hosted adapter behind the web proxy                                  | Next.js API routes                                                                       |
...
The Compose reference provisions PostgreSQL 16, keeps the API-owned migration service behind the explicit `migration` profile, starts web only after the API health check, publishes only the web port, and schedules the bounded retention purge daily through the API. Operators apply migrations separately before starting application traffic and still own HTTPS termination, secret management, backups, monitoring, host hardening, and restore drills. The shared API Prisma factory sets `max: 5` for the `pg` pool: Worker requests create and disconnect one request-scoped client in `finally`, scheduled purges do the same, and Bun creates one process-scoped client and disconnects it on `SIGTERM`/`SIGINT`.
...
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_REQUIRE_TLS`, `SMTP_USER`, `SMTP_PASSWORD`, `AUTH_EMAIL_FROM`, `AUTH_EMAIL_FROM_NAME` | Bun, self-hosted, and Cloudflare Worker API passwordless runtimes | Server-only Nodemailer SMTP settings shared by every API runtime.                                                                                                                                 |
...
| `DATABASE_URL`                                                                                                                       | Bun API runtime                                                   | Runtime Prisma URL; use TLS in production. Cloudflare Worker requests use Hyperdrive instead.                                                                                                     |
...
Run `pnpm run verify:deployment-config` before deployment. Use `VERIFY_DEPLOYMENT_PRODUCTION=1` to enforce production requirements; use `DEPLOYMENT_TARGET=worker` for the Cloudflare API and `DEPLOYMENT_TARGET=bun` (the default) for self-hosting. It validates URL shape, backend configuration, API proxy credentials, secret length, and conditional variables without printing their values. Worker validation rejects direct database URLs and both targets validate the shared SMTP runtime configuration. The web validation rejects API-only variables, and the Vercel build runs that validation before `next build`.

For Cloudflare, provision the same API-only values as Worker vars/secrets before deploying; do not put SMTP or database values in Vercel. At minimum, configure `PROXY_SECRET`, `AUTH_MAGIC_LINK_SECRET`, `AUTH_SESSION_SECRET`, `TURNSTILE_SECRET_KEY`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_REQUIRE_TLS`, `SMTP_USER`, `SMTP_PASSWORD`, `AUTH_EMAIL_FROM`, and `AUTH_EMAIL_FROM_NAME` with Wrangler, plus `HYPERDRIVE` and `CRON_SECRET`. Add `PASSKEY_RP_ID`, `PASSKEY_ORIGIN`, and `AUTH_ADMITTED_EMAILS` when those optional features are enabled. Use `DEPLOYMENT_TARGET=worker VERIFY_DEPLOYMENT_PRODUCTION=1 pnpm run verify:deployment-config` against a redacted local deployment environment; never print or commit the values.
