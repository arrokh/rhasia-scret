...
| `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN`          | Optional PostHog browser analytics                         | _(empty)_                                                                                         | Optional public project token. Leave it empty unless analytics is intentionally enabled.                                                                                                                                                              |
...
| `NEXT_PUBLIC_CLOUDFLARE_WEB_ANALYTICS_TOKEN` | Optional Cloudflare Web Analytics                          | _(empty)_                                                                                         | Optional public beacon token. Leave empty for default-off analytics. Never use this variable for a server credential.                                                                                                                                 |
...
| `OIDC_CLIENT_SECRET`                         | `AUTH_BACKEND=oidc`                                        | _(secret-manager value)_                                                                          | Required server-only client secret. Never expose it to browser or native environment variables.                                                                                                                                                       |
...
| `OIDC_SESSION_SECRET`                        | `AUTH_BACKEND=oidc`                                        | _(at least 32 random characters)_                                                                 | Required server-only signing secret. Configuration rejects values shorter than 32 characters.                                                                                                                                                         |
...
| `CRON_SECRET`                                | Every deployment that runs retention purge                 | _(at least 32 random characters)_                                                                 | Server-only bearer secret. The retention route returns `503` when missing/too short and `401` for an invalid bearer value. Store it in the scheduler secret manager, not shell history or source control.                                             |
