...
## Expected behaviour
Show after a successfully decrypted Vault contains no active Authenticator Accounts. The add action is available to a Personal Vault user, Shared Vault Owner, or Viewer with effective add permission; another Viewer sees an access-appropriate read-only empty state. Explain the supported client-only import methods without implying that raw QR data or TOTP secrets leave the browser.
