...
- empty states
- recovery guidance
- Remembered Browser enrollment confirmation
...
[Vault Passphrase or, where supported, Local Verification control]

Unlock vault
```

Use gold for the active unlock control and primary action. Do not use technical cryptography terms here; do not present the Vault Unlock Secret as a PIN.
...
- account capabilities: neutral ink-soft
...
```text
This member can view and copy codes; account changes follow the configured permissions.
```
...
- Passkey-Assisted Recovery where enrolled

#### Account

- Email
- Remembered Browser where supported
- Sign out
...
Use human-readable capability labels when showing Effective Shared Vault Account Permissions. Avoid uppercase technical role names.
...
```text
You’re offline.
Your saved codes still work. Account changes are unavailable while offline.
```
...
```text
Unable to refresh encrypted vault data.
...
- sync state
- conflict state
- Remembered Browser and Passkey-Assisted Recovery
- encrypted backup
