...
The main-push workflow also runs the smoke suite for Chromium, Firefox, and WebKit in the same six-cell matrix. Each cell has its own PostgreSQL service and browser cache entry, and the production PWA suite remains a separate job that reuses the verified build where applicable.
