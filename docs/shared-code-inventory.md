...
| Secure Share Link material creation/redemption, HTTP protocol, and injected workflow ports | Extract now | `@rhasia-scret/client-vault-core` | Cryptographic workflow and ciphertext-only HTTP semantics are above injected client crypto and `AuthenticatedTransport` ports; navigation and sharing effects stay platform-specific. |
