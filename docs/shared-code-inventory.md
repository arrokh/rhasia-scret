...
| Authenticated crypto envelope contracts, unlock parameters, archive protocol, and injected crypto ports | Extract now | `@rhasia-scret/client-vault-core` | Client-only orchestration above injected ports; browser/native primitives remain in their applications. No secret is logged or persisted by the package. |
| Encrypted account payload and workspace models/ports | Extract now | `@rhasia-scret/client-vault-core` | Shared encrypted DTO and headless workspace behavior are used by both application consumers; labels and plaintext remain in client memory. |
| Offline encrypted bundle parsing and snapshot ports | Extract now | `@rhasia-scret/client-vault-core` | Read-only encrypted bundle contract is platform-neutral; IndexedDB and native storage implementations stay in application infrastructure. |
| Secure Share Link material creation/redemption and injected workflow ports | Extract now | `@rhasia-scret/client-vault-core` | Cryptographic workflow is above injected client crypto and transport ports; HTTP and navigation adapters stay platform-specific. |
...
| Prisma repositories, Next route handlers, server session/auth adapters, and server audit/retention code | Keep platform-specific | `apps/web` | Server-side application data access and authorization must not depend on client crypto/decryption or OTP runtime adapters. |
