...
- `apps/api` owns the Hono/Cloudflare Worker service, canonical `/v1/**` routes, server application modules, Prisma schema/migrations, persistence, email/auth adapters, retention scheduling, and API tests/configuration. Bun is the supported self-hosted runtime adapter.
