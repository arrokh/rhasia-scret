...
test.describe("encrypted read-only offline PWA", () => {
  test("uses the configured manifest and boots only the public offline shell from a static cache", async ({ page, context, browserName }) => {
    ...
    expect(manifest).toMatchObject({ name: "rhasia-scret", short_name: "rhasia-scret", display: "standalone", orientation: "portrait" });
    ...
    const marker = await page.evaluate(async () => new Promise<string | undefined>((resolve, reject) => {
      const request = indexedDB.open("pwa-activation-preservation", 1);
      request.onerror = () => reject(request.error);
      request.onsuccess = () => {
        const transaction = request.result.transaction("marker", "readonly");
        const get = transaction.objectStore("marker").get("state");
        get.onsuccess = () => { request.result.close(); resolve(get.result as string | undefined); };
  ...
  test("unlocks ciphertext offline, generates OTPs, retains stale data on failures, and reconciles revocation only after complete success", async ({ page, context, browserName }) => {
    ...
    for (const plaintext of [...initial.accounts.flatMap((account) => [account.issuer, account.accountName, account.secretBase64]), ...initial.vaultNames, ...initial.sensitiveValues, ...copiedCodes]) {
...
type FixtureBundle = { bundle: Record<string, unknown>; secret: string; accounts: FixtureAccount[]; vaultNames: string[]; sensitiveValues: string[] };

async function encryptedFixture({ includeViewer, synchronizedAt }: { includeViewer: boolean; synchronizedAt: string }): Promise<FixtureBundle> {
  const secret = "correct horse battery staple";
  const salt = Uint8Array.from({ length: 16 }, (_, index) => index + 1);
  const unlockKey = await argon2id({ password: secret, salt, parallelism: 1, iterations: 3, memorySize: 64 * 1024, hashLength: 32, outputType: "binary" });
  ...
  if (includeViewer) sharedVaults.push(await sharedVault("viewer_vault", "Viewer Vault", "VIEWER", viewerKey, viewerAccount.record));
  const bundle = {
    schemaVersion: 1,
  ...
  return {
    bundle,
    secret,
    accounts: includeViewer ? [personalAccount.fixture, ownerAccount.fixture, viewerAccount.fixture] : [personalAccount.fixture, ownerAccount.fixture],
...
async function seedBundle(page: import("@playwright/test").Page, bundle: Record<string, unknown>) {
  await page.evaluate(async (value) => new Promise<void>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      if (!request.result.objectStoreNames.contains("encrypted-snapshots")) request.result.createObjectStore("encrypted-snapshots", { keyPath: "profileId" });
      if (!request.result.objectStoreNames.contains("remembered-browsers")) request.result.createObjectStore("remembered-browsers", { keyPath: "profileId" });
    };
    request.onerror = () => reject(request.error);
    request.onsuccess = () => {
...
async function storedBundleText(page: import("@playwright/test").Page): Promise<string> {
  return page.evaluate(async () => new Promise<string>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      all.onsuccess = () => { request.result.close(); resolve(JSON.stringify(all.result)); };
