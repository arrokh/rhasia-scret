...
test("keeps the Vault shell stable while only the selected async tab shows a placeholder", async ({ page }) => {
  ...
  await page.route("**/api/shared-vaults/shared-preview/participants**", async (route) => { participantsRoute = route; });
  await page.route("**/api/vaults/shared-preview/audit-events**", async (route) => { auditRoute = route; });
  ...
  await participantsRoute?.fulfill({
    ...
    body: JSON.stringify({ owner: { id: "owner-preview", email: "owner@local.invalid" }, vaultDefaultAccountPermissions: { canAddAccounts: false, canEditAccounts: false, canDeleteAccounts: false }, vaultDefaultAccountPermissionsRevision: 1, participants: [], nextCursor: null })
  });
  ...
  await expect(page.getByRole("heading", { level: 1, name: "Brankas" })).toBeVisible();
  await auditRoute?.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ events: [], nextCursor: null }) });
