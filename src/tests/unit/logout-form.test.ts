...
describe("LogoutForm", () => {
  ...
  afterEach(async () => { mocks.mutateAsync.mockClear(); await act(async () => root?.unmount()); document.body.innerHTML = ""; });
  ...
  it("uses a shadcn menu and requires confirmation before terminating sessions", async () => {
    ...
    const confirm = [...(dialog?.querySelectorAll<HTMLButtonElement>("button") ?? [])].find((button) => button.textContent === "Keluar");
