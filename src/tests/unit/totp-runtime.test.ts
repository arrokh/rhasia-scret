...
describe("TOTP runtime", () => {
  it("parses a supported normalized TOTP URI", () => {
    ...
    expect(configuration).toMatchObject({ issuer: "Example", accountName: "alice", algorithm: "SHA-1", digits: 8, period: 30 });
  ...
  it("formats an RFC 4226 dynamic-truncation value", async () => {
    ...
    const code = await generateTotp(configuration, {
      sign: async () => new Uint8Array([0xcc, 0x93, 0xcf, 0x18, 0x50, 0x8d, 0x94, 0x93, 0x4c, 0x64, 0xb6, 0x5d, 0x8b, 0xa7, 0x66, 0x7f, 0xb7, 0xcd, 0xe4, 0xb0])
    }, new Date(0));
