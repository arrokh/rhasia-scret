...
describe("Vault Unlock Key derivation", () => {
  ...
  it("falls back to the unchanged inline derivation when the module Worker cannot load", async () => {
    ...
    class FailingWorker {
      ...
      terminate() { workerState.terminated = true; }
    }
    ...
    expect(mocks.argon2id).toHaveBeenCalledWith(expect.objectContaining({ iterations: 3, memorySize: 64 * 1024, parallelism: 1 }));
  ...
  it("uses a short-lived module Worker and transfers only copied derivation bytes", async () => {
    const workerState: { url?: URL; options?: WorkerOptions; terminated: boolean; transfers: number } = { terminated: false, transfers: 0 };
    class FakeWorker {
      onmessage: ((event: MessageEvent<{ ok: true; key: ArrayBuffer }>) => void) | null = null;
      onerror: (() => void) | null = null;
      constructor(url: URL, options: WorkerOptions) { workerState.url = url; workerState.options = options; }
      postMessage(_: { password: ArrayBuffer; salt: ArrayBuffer }, transfer: Transferable[]) {
        workerState.transfers = transfer.length;
        queueMicrotask(() => this.onmessage?.(new MessageEvent("message", { data: { ok: true, key: new Uint8Array(32).fill(9).buffer } })));
      }
      terminate() { workerState.terminated = true; }
    }
