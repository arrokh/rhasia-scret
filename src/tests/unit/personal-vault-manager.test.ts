...
describe("Personal Vault management", () => {
  ...
  it("uses the Shared Vault management structure without Shared-only controls", async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ events: [{ id: "archive-event", eventType: "ARCHIVE_IMPORTED", targetId: null, actorUserId: "owner-1", actorEmail: "owner@example.test", createdAt: "2026-07-27T12:00:00.000Z" }], nextCursor: null }) });
    vi.stubGlobal("fetch", fetchMock);
    await act(async () => root?.render(createElement(TestQueryProvider, null, createElement(PersonalVaultDetails, {
