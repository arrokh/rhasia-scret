...
import { decryptPayloadWithContext, encryptPayload, generateSymmetricKey, serializeEncryptedEnvelope, deserializeEncryptedEnvelope } from "@/modules/crypto/infrastructure/browser-crypto-envelope";
...
describe("unlockPersonalVault", () => {
  ...
  it("migrates a legacy profile envelope during unlock instead of rejecting a correct secret", async () => {
    ...
    try {
      ...
      expect(deserializeEncryptedEnvelope(unlocked.migratedProfile?.wrappedUserRootKey ?? new Uint8Array()).version).toBe(2);
      expect(deserializeEncryptedEnvelope(unlocked.migratedProfile?.encryptedPersonalVaultKey ?? new Uint8Array()).version).toBe(2);
      await expect(unlockPersonalVault(secret, unlocked.migratedProfile as typeof profile)).resolves.toMatchObject({ personalVaultKey });
      await expect(decryptPayloadWithContext(unlocked.userRootKey, deserializeEncryptedEnvelope(unlocked.migratedProfile?.encryptedPersonalVaultKey ?? new Uint8Array()), { purpose: "vault-key-wrap", payloadType: "vault-encryption-key", keyVersion: 1 })).resolves.toEqual(personalVaultKey);
    } finally {
      unlockKey.fill(0);
      userRootKey.fill(0);
