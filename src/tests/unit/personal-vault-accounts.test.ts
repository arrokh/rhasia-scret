...
describe("PersonalVaultAccounts", () => {
  ...
  it("renders only the aggregated account list with vault provenance and a dedicated add-page link", async () => {
    mocks.loadUnlockedVaultWorkspace.mockResolvedValue({
      ...
      accounts: [
        { id: "account-1", vaultId: "personal-1", vaultName: "Brankas Pribadi", vaultType: "PERSONAL", issuer: "Example", accountName: "personal@example.test", secret: Uint8Array.of(4), algorithm: "SHA-1", digits: 6, period: 30 },
        { id: "account-2", vaultId: "shared-1", vaultName: "Tim Operasional", vaultType: "SHARED", issuer: "Work", accountName: "work@example.test", secret: Uint8Array.of(5), algorithm: "SHA-1", digits: 6, period: 30 }
      ]
    });
    const container = document.createElement("div");
    root = createRoot(container);
    await act(async () => root?.render(createElement(PersonalVaultAccounts, { vaultId: "personal-1" })));
