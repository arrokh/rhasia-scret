...
describe("POST /api/vault-imports contract", () => {
  it("passes only bounded ciphertext and opaque metadata to one atomic repository call", async () => {
    const importArchive = vi.fn<EncryptedVaultImportRepository["import"]>().mockResolvedValue({ status: "IMPORTED", vaultId, accountIds: [accountId], vaultCreated: true });
    const handler = createHandler(importArchive);
    const body = {
      destination: { kind: "NEW_SHARED", vaultId, encryptedName: ciphertext, encryptedOwnerVaultKey: ciphertext, encryptionVersion: 1 },
    ...
    await expect(response.json()).resolves.toEqual({ vaultId, accountIds: [accountId], vaultCreated: true, replayed: false });
  ...
  it("rejects incomplete or conflicting imports without accepting partial writes", async () => {
    ...
    const handler = createHandler(importArchive);
    const incomplete = await handler(request({ destination: { kind: "EXISTING", vaultId: "vault-1", vaultType: "PERSONAL" }, accounts: [{ id: accountId, encryptionVersion: 1 }] }));
    expect(incomplete.status).toBe(400);
    expect(importArchive).not.toHaveBeenCalled();

    importArchive.mockResolvedValueOnce({ status: "CONFLICT" });
    const conflict = await handler(request({ destination: { kind: "EXISTING", vaultId: "vault-1", vaultType: "PERSONAL" }, accounts: [{ id: accountId, encryptedPayload: ciphertext, encryptionVersion: 1 }] }));
  ...
  it("rejects oversized requests before invoking persistence", async () => {
    const importArchive = vi.fn<EncryptedVaultImportRepository["import"]>();
    const handler = createHandler(importArchive);
    const response = await handler(new NextRequest("http://localhost/api/vault-imports", { method: "POST", body: "{}", headers: { "content-length": String(8 * 1024 * 1024 + 1) } }));
...
function request(body: unknown): NextRequest {
  return new NextRequest("http://localhost/api/vault-imports", { method: "POST", body: JSON.stringify(body), headers: { "content-type": "application/json" } });
}
