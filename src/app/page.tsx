...
function ConnectionFlow({ t }: { t: LandingTranslator }) {
  ...
  return <div className="overflow-visible rounded-xl border border-border bg-muted/25 p-3 sm:p-6">
    ...
    <div className="comparison-flow-layout mt-5 grid min-w-0 overflow-visible grid-cols-[minmax(0,1fr)_5rem_minmax(0,1fr)] gap-2 min-[360px]:grid-cols-[minmax(0,1fr)_7.5rem_minmax(0,1fr)] min-[400px]:grid-cols-[minmax(0,1fr)_6.5rem_minmax(0,1fr)] min-[400px]:gap-3 min-[640px]:grid-cols-[minmax(0,1fr)_8.5rem_minmax(0,1fr)] min-[640px]:gap-4 lg:mt-6 lg:grid-cols-[minmax(0,1fr)_minmax(12rem,0.75fr)_minmax(0,1fr)] lg:items-center lg:gap-10">
      <div className="grid min-w-0 overflow-visible justify-items-center self-center gap-2 lg:relative lg:flex lg:self-stretch lg:items-center lg:justify-stretch"><p className="whitespace-nowrap text-center text-[0.48rem] font-bold tracking-[0.08em] text-muted-foreground uppercase lg:absolute lg:top-0 lg:left-0 lg:text-left lg:text-xs lg:tracking-[0.12em]">{t("comparison.flow.devices")}</p><div className="comparison-flow-devices-grid grid min-w-0 w-full overflow-visible justify-items-center gap-2 lg:flex lg:flex-col lg:justify-items-stretch lg:gap-3">{devices.map(([Icon, device]) => <div key={device} className="comparison-flow-device relative grid justify-items-center gap-1 rounded-lg border border-border bg-card p-2 text-center text-[0.65rem] font-bold leading-4 text-ink-strong max-lg:size-12 max-lg:justify-self-center max-lg:place-items-center max-lg:p-0 lg:flex lg:items-center lg:gap-3 lg:p-3 lg:text-sm lg:text-left"><span className="relative z-10 grid size-7 place-items-center rounded-md bg-primary/10 text-primary lg:size-9"><Icon className="size-4 lg:size-5" aria-hidden="true" /></span><span className="relative z-10 max-lg:hidden">{t(`comparison.flow.device.${device}`)}</span><span className="comparison-flow-branch-dot" aria-hidden="true" /></div>)}</div></div>
      ...
      <div className="comparison-flow-services grid min-w-0 overflow-visible justify-items-center self-center gap-2 lg:relative lg:flex lg:self-stretch lg:items-center lg:justify-stretch"><p className="text-center text-[0.6rem] leading-3 font-bold tracking-[0.1em] text-muted-foreground uppercase lg:absolute lg:top-0 lg:left-0 lg:text-left lg:text-xs lg:leading-normal lg:tracking-[0.12em]">{t("comparison.flow.services")}</p><div className="comparison-flow-services-grid grid min-w-0 w-full overflow-visible justify-items-center gap-2 lg:flex lg:flex-col">{featuredServices.map(([Icon, service]) => <div key={service} className="comparison-flow-service relative grid justify-items-center gap-1 rounded-lg border border-border bg-card p-2 text-center text-[0.65rem] font-bold leading-4 text-ink-strong max-lg:size-12 max-lg:justify-self-center max-lg:place-items-center max-lg:p-0 lg:inline-flex lg:w-full lg:items-center lg:gap-2 lg:px-3 lg:py-2 lg:text-sm lg:text-left"><span className="relative z-10 grid size-7 place-items-center rounded-md bg-muted text-ink-strong"><Icon className="size-4" aria-hidden="true" /></span><span className="relative z-10 max-lg:hidden">{t(`comparison.flow.service.${service}`)}</span><span className="comparison-flow-branch-dot" aria-hidden="true" /></div>)}<div className="comparison-flow-service comparison-flow-service-summary relative grid justify-items-center gap-1 rounded-lg border border-border bg-card p-2 text-center text-[0.65rem] font-bold leading-4 text-ink-strong max-lg:size-12 max-lg:justify-self-center max-lg:place-items-center max-lg:p-0 lg:flex lg:w-full lg:items-center lg:gap-2 lg:px-3 lg:py-2 lg:text-left" aria-label={remainingServices.map(([, service]) => t(`comparison.flow.service.${service}`)).join(", ")}><span className="comparison-flow-summary-icons relative z-10 hidden gap-1 lg:flex">{remainingServices.slice(0, 6).map(([Icon, service]) => <span key={service} className="grid size-6 place-items-center rounded-md bg-muted text-ink-strong"><Icon className="size-3.5" aria-hidden="true" /></span>)}</span><span className="relative z-10 grid size-7 place-items-center rounded-md bg-muted text-ink-strong"><Ellipsis className="size-4" aria-hidden="true" /></span><span className="comparison-flow-branch-dot" aria-hidden="true" /></div></div></div>
    </div>
