...
import { LockKeyhole, Plus, ShieldKeyhole } from "lucide-react";
...
export default async function MobilePreviewPage() {
  ...
  return <AppPage>
    ...
    <SurfaceCard className="grid gap-5 p-4 sm:p-5" aria-label={t("listLabel")}>
      <div className="flex items-center gap-2"><Button variant="outline" asChild><Link href="/vaults/manage" aria-label={t("vaults")} title={t("vaults")}><LockKeyhole aria-hidden="true" /><span className="hidden sm:inline">{t("vaults")}</span></Link></Button><Button variant="outline" aria-label={t("securityAction")}><ShieldKeyhole aria-hidden="true" /><span>{t("securityAction")}</span></Button><Button asChild className="ml-auto"><Link href="/vaults/accounts/new" aria-label={t("addLabel")}><Plus /><span className="hidden sm:inline">{t("add")}</span></Link></Button></div>
