...
export default function MobilePreviewPage() {
  ...
  return <AppPage>
    <PageHeader title="Akun autentikator" description="Tidak ada materi akun, passphrase, OTP, atau kunci yang digunakan dalam data contoh ini." action={<LogoutForm email="preview@local.invalid" />} />
    <SurfaceCard className="grid gap-5 p-4 sm:p-5" aria-label="Daftar akun autentikator">
      <div className="flex items-center gap-2"><Button variant="outline" asChild><Link href="/vaults/manage" aria-label="Brankas" title="Brankas"><UsersRound aria-hidden="true" /><span className="hidden sm:inline">Brankas</span></Link></Button><Button variant="outline" size="icon" aria-label="Keamanan brankas"><ShieldKeyhole /></Button><Button asChild className="ml-auto"><Link href="/vaults/accounts/new" aria-label="Tambahkan akun autentikator"><Plus /><span className="hidden sm:inline">Tambah akun</span></Link></Button></div>
