...
export async function POST(request: NextRequest, { params }: { params: Promise<{ vaultId: string }> }) {
  try { const user = await owner(); if (!user) return NextResponse.json({ error: "unauthenticated" }, { status: 401 }); const data = payload.safeParse(await request.json()); if (!data.success) return NextResponse.json({ error: "invalid_account" }, { status: 400 }); const { vaultId } = await params; const account = await new PrismaSharedOwnerAccountRepository().create(user.id, vaultId, Buffer.from(data.data.encryptedPayload, "base64"), data.data.encryptionVersion); return NextResponse.json({ id: account.id, revision: account.revision }, { status: 201 }); } catch (error) { return accessError(error); }
}
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ vaultId: string }> }) {
  try { const user = await owner(); if (!user) return NextResponse.json({ error: "unauthenticated" }, { status: 401 }); const data = updateSchema.safeParse(await request.json()); if (!data.success) return NextResponse.json({ error: "invalid_account" }, { status: 400 }); const { vaultId } = await params; const account = await new PrismaSharedOwnerAccountRepository().update(user.id, vaultId, data.data.accountId, data.data.expectedRevision, Buffer.from(data.data.encryptedPayload, "base64"), data.data.encryptionVersion); return account ? NextResponse.json({ id: account.id, revision: account.revision }) : NextResponse.json({ error: "stale_revision" }, { status: 409 }); } catch (error) { return accessError(error); }
}
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ vaultId: string }> }) {
  try { const user = await owner(); if (!user) return NextResponse.json({ error: "unauthenticated" }, { status: 401 }); const data = deleteSchema.safeParse(await request.json()); if (!data.success) return NextResponse.json({ error: "invalid_account" }, { status: 400 }); const { vaultId } = await params; return await new PrismaSharedOwnerAccountRepository().delete(user.id, vaultId, data.data.accountId, data.data.expectedRevision) ? new NextResponse(null, { status: 204 }) : NextResponse.json({ error: "stale_revision" }, { status: 409 }); } catch (error) { return accessError(error); }
}
