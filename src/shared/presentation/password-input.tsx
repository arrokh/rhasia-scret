...
export function PasswordInput({ label, visible, onToggleVisibility, className, ...props }: Omit<React.ComponentProps<typeof Input>, "type"> & { label: string; visible: boolean; onToggleVisibility: () => void }) {
  ...
  return <div className="relative">
    ...
    <Button variant="ghost" size="icon" type="button" className="absolute inset-y-0 right-1 my-auto text-muted-foreground transition-none active:!translate-y-0" onClick={onToggleVisibility} disabled={props.disabled} aria-label={visibilityLabel} title={visibilityLabel}>
