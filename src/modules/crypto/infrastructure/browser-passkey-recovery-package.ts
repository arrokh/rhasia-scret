...
function parsePackage(packageBytes: Uint8Array): { prfSalt: Uint8Array; encryptedRecoveryWrappingKey: string; encryptedUserRootKey: string } {
  if (packageBytes.length === 0 || packageBytes.length > MAX_PACKAGE_BYTES) throw new Error("Passkey recovery package is invalid.");
  let parsed: unknown;
  try { parsed = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(packageBytes)); } catch { throw new Error("Passkey recovery package is invalid."); }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Passkey recovery package is invalid.");
  const data = parsed as Record<string, unknown>;
  const expected = ["encryptedRecoveryWrappingKey", "encryptedUserRootKey", "prfSalt", "version"];
  const keys = Object.keys(data).sort();
  if (keys.length !== expected.length || keys.some((key, index) => key !== expected[index]) || data.version !== VERSION || typeof data.prfSalt !== "string" || typeof data.encryptedRecoveryWrappingKey !== "string" || typeof data.encryptedUserRootKey !== "string") throw new Error("Passkey recovery package is invalid.");
  let prfSalt: Uint8Array;
  try { prfSalt = base64ToBytes(data.prfSalt); } catch { throw new Error("Passkey recovery package is invalid."); }
  if (prfSalt.length !== KEY_LENGTH) { prfSalt.fill(0); throw new Error("Passkey recovery package is invalid."); }
  return { prfSalt, encryptedRecoveryWrappingKey: data.encryptedRecoveryWrappingKey, encryptedUserRootKey: data.encryptedUserRootKey };
}
