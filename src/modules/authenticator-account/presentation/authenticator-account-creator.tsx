...
export function AuthenticatorAccountCreator({ personalVaultId, preferredVaultId }: { personalVaultId: string; preferredVaultId?: string }) {
  ...
  return <>
    ...
    <form noValidate className="grid gap-5 bg-muted/30 p-5 sm:p-6" onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void accountForm.handleSubmit(); }}>
      ...
      <div className="grid grid-cols-2 gap-3"><Button variant="outline" asChild><Link href="/vaults">Batal</Link></Button><accountForm.Subscribe selector={(state) => state.isSubmitting}>{(isSubmitting) => <Button type="submit" disabled={!online || isSubmitting} aria-busy={isSubmitting}>{isSubmitting ? "Menyimpan…" : "Simpan akun"}</Button>}</accountForm.Subscribe></div>
    </form>
