export class EncryptedAuthenticatorAccount {
  public constructor(
    ...
    if (!id || !vaultId || !encryptedPayload.length) throw new Error("Encrypted account identity and payload are required.");
    if (!Number.isInteger(encryptionVersion) || encryptionVersion < 1) throw new Error("An encryption version is required.");
