...
export class PrismaPasskeyRecoveryRepository {
  public async issueChallenge(userId: string, purpose: "REGISTRATION" | "AUTHENTICATION", challenge: string): Promise<void> {
    await prisma.$transaction([prisma.passkeyRecoveryChallenge.deleteMany({ where: { userId, purpose } }), prisma.passkeyRecoveryChallenge.create({ data: { userId, purpose, challenge, expiresAt: new Date(Date.now() + CHALLENGE_TTL_MS) } })]);
  }

  public async consumeChallenge(userId: string, purpose: "REGISTRATION" | "AUTHENTICATION"): Promise<string | null> {
    const record = await prisma.passkeyRecoveryChallenge.findFirst({ where: { userId, purpose, expiresAt: { gt: new Date() }, }, orderBy: { createdAt: "desc" } });
