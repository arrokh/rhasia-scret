...
export function SharedVaultCreator({ userRootKey }: { userRootKey: Uint8Array }) {
  ...
  return <form className="auth-form" onSubmit={submit}><label htmlFor="shared-vault-name">New Shared Vault Name</label><input id="shared-vault-name" value={name} onChange={(event) => setName(event.target.value)} required /><button className="primary-button" type="submit">Create Shared Vault</button>{status && <p aria-live="polite">{status}</p>}</form>;
}
